...
((() => {
	const parameters = PluginManager.parameters('Debug_ReportMemory');
	let pixels = toNumber(parameters['Max Pixels In MPix'], 10);
	const communityPixels = PluginManager.parameters('Community_Basic')['cacheLimit'];

	if (communityPixels) {
		pixels = toNumber(communityPixels, 10);
		console.log(`Community_Basic plugin has been installed, so it overwrites the setting value of cacheLimit. cacheLimit: ${pixels}MPix`);
	}
	...
	function usedSize(item, bitmaps) {
		...
		if (item && item.children) {
			totalSize += item.children.reduce((sum, child) => sum + usedSize(child, bitmaps), 0);
		}
		return totalSize;
	}

	function updateInfo() {
		let content = `usedSize: ${toMPix(usedSize(SceneManager._scene, []))}<br>`;
	...
	ImageCache.prototype._countRequest = function () {
		...
		return Object.keys(this._items)
			.map(key => items[key].bitmap)
			.filter(bitmap => bitmap.isRequestOnly())
			.length;
	};
	...
	ImageCache.prototype._countReserved = function () {
		...
		return Object.keys(this._items)
			.map(key => items[key].reservationId)
			.filter(id => id)
			.length;
	};
}))();
