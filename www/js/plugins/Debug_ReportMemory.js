...
(() => {
	...
	const communityPixels =
		PluginManager.parameters('Community_Basic')['cacheLimit'];

	if (communityPixels) {
		pixels = toNumber(communityPixels, 10);
		console.log(
			`Community_Basic plugin has been installed, so it overwrites the setting value of cacheLimit. cacheLimit: ${pixels}MPix`
		);
	}
	...
	function usedSize(item, bitmaps) {
		...
		if (item && item.children) {
			totalSize += item.children.reduce(
				(sum, child) => sum + usedSize(child, bitmaps),
				0
			);
		}
		return totalSize;
	...
	ImageCache.prototype._countRequest = function () {
		...
		return Object.keys(this._items)
			...
			.filter((bitmap) => bitmap.isRequestOnly()).length;
	};
