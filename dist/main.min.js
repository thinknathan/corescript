...
(function () {
  ...
  class Utils {
     ...
     */ static getFps() {
      if (Utils._fpsChecked || Utils._fpsIsBusyCounting) return Utils._fps;
      {
        ...
        const s = (a) => {
          e <= 180
            ? (e++, (i += 1e3 / (a - t)), (t = a), requestAnimationFrame(s))
            : (Utils._fps !== 1 / 0 &&
                ((Utils._fps = i / e),
  ...
  class Decrypter {
    ...
    static decryptArrayBuffer(t) {
      ...
      let i;
      const s = this.SIGNATURE + this.VER + this.REMAIN,
        a = new Uint8Array(16);
      for (i = 0; i < this._headerlength; i++)
        a[i] = parseInt(`0x${s.substr(2 * i, 2)}`, 16);
      for (i = 0; i < this._headerlength; i++)
        if (e[i] !== a[i]) throw new Error("Header is wrong");
  ...
  class WebAudio {
    ...
    _readOgg(t) {
      ...
      for (; e < t.length && "OggS" === this._readFourCharacters(t, e); ) {
        ...
        const s = t[e++],
          a = [];
        for (let i = 0; i < s; i++) a.push(t[e++]);
        for (let r = 0; r < s; r++) {
          if ("vorb" === this._readFourCharacters(t, e + 1)) {
            const n = t[e];
            if (1 === n) this._sampleRate = this._readLittleEndian(t, e + 12);
            else if (3 === n) {
              let i = 0;
              for (; r < s && ((i += a[r]), !(a[r] < 255)); r++);
      ...
      if ("ftyp" === this._readFourCharacters(t, 4)) {
        ...
        for (; e < t.length; ) {
          const i = this._readBigEndian(t, e),
            s = this._readFourCharacters(t, e + 4);
          if ("moov" === s) e += 8;
          else if (
            ("mvhd" === s &&
              (this._sampleRate = this._readBigEndian(t, e + 20)),
            ("udta" !== s && "meta" !== s) || this._readMetaData(t, e, i),
            (e += i),
            i <= 1)
          )
            break;
    ...
    _readMetaData(t, e, i) {
      for (let s = e; s < e + i - 10; s++)
        if ("LOOP" === this._readFourCharacters(t, s)) {
          let e = "";
  ...
  class AudioManager {
    ...
    static playEncryptedBgm({ name: t }, e) {
      ...
      let s = `${this._path}bgm/${encodeURIComponent(t)}${i}`;
      s = Decrypter.extToEncryptExt(s);
    }
    ...
    static updateCurrentBgm({ name: t, volume: e, pitch: i, pan: s }, a) {
      this._currentBgm = { name: t, volume: e, pitch: i, pan: s, pos: a };
    }
    static stopBgm() {
      this._bgmBuffer &&
    ...
    static updateCurrentBgs({ name: t, volume: e, pitch: i, pan: s }, a) {
      this._currentBgs = { name: t, volume: e, pitch: i, pan: s, pos: a };
    }
    static stopBgs() {
      this._bgsBuffer &&
    ...
    static createBuffer(t, e) {
      const i = this.audioFileExt(),
        s = `${this._path + t}/${encodeURIComponent(e)}${i}`,
        a = new WebAudio(s);
      return this._callCreationHook(a), a;
    }
    static checkErrors() {
  ...
  class Input {
    ...
    static _updateGamepadState(t) {
      ...
      a[1] < -0.5 ? (i[12] = !0) : a[1] > 0.5 && (i[13] = !0),
        a[0] < -0.5 ? (i[14] = !0) : a[0] > 0.5 && (i[15] = !0);
  ...
  class TouchInput {
    ...
    static _onLeftButtonDown({ pageX: t, pageY: e }) {
      const i = Graphics$1.pageToCanvasX(t),
        s = Graphics$1.pageToCanvasY(e);
      Graphics$1.isInsideCanvas(i, s) &&
    ...
    static _onRightButtonDown({ pageX: t, pageY: e }) {
      const i = Graphics$1.pageToCanvasX(t),
        s = Graphics$1.pageToCanvasY(e);
      Graphics$1.isInsideCanvas(i, s) && this._onCancel(i, s);
    }
    ...
    static _onMouseUp({ button: t, pageX: e, pageY: i }) {
      if (0 === t) {
        const t = Graphics$1.pageToCanvasX(e),
          s = Graphics$1.pageToCanvasY(i);
        (this._mousePressed = !1), this._onRelease(t, s);
      }
    }
    static _onWheel(t) {
    ...
    static _onTouchStart(t) {
      for (const e of t.changedTouches) {
        const i = Graphics$1.pageToCanvasX(e.pageX),
          s = Graphics$1.pageToCanvasY(e.pageY);
        Graphics$1.isInsideCanvas(i, s) &&
          ((this._screenPressed = !0),
          (this._pressedTime = 0),
          t.touches.length >= 2 ? this._onCancel(i, s) : this._onTrigger(i, s),
          t.preventDefault());
      }
    ...
    static _onTouchMove({ changedTouches: t }) {
      for (const e of t) {
        const t = Graphics$1.pageToCanvasX(e.pageX),
          i = Graphics$1.pageToCanvasY(e.pageY);
        this._onMove(t, i);
      }
    }
    ...
    static _onPointerDown(t) {
      if ("touch" === t.pointerType && !t.isPrimary) {
        const e = Graphics$1.pageToCanvasX(t.pageX),
          i = Graphics$1.pageToCanvasY(t.pageY);
        Graphics$1.isInsideCanvas(e, i) &&
  ...
  class SceneManager {
    ...
    static onError({ message: t, filename: e, lineno: i }) {
      if (e || i)
        try {
          this.stop(),
            Graphics$1.printError("Error", t),
            AudioManager.stopAll();
        } catch (t) {}
    }
    static catchException(t) {
      t instanceof Error
        ? (Graphics$1.printError(t.name, t.message),
          Graphics$1.printErrorDetail(t))
        : Graphics$1.printError("UnknownError", t),
        AudioManager.stopAll(),
        this.stop();
    }
    static updateMain() {
      ...
      if (Utils.isNwjs()) {
        ...
        if ("darwin" === process.platform && !e.menu) {
          const i = new t.Menu({ type: "menubar" }),
            s = { hideEdit: !0, hideWindow: !0 };
          i.createMacBuiltin("Game", s), (e.menu = i);
        }
      }
    }
    ...
    static onSceneStart() {
      Graphics$1.callGC(), Graphics$1.endLoading();
    }
  ...
  class ResourceHandler {
    ...
    static createLoader(t, e, i, s = this._defaultRetryInterval) {
      const a = this._reloaders;
      let r = 0;
      return () => {
        r < s.length
          ? (setTimeout(e, s[r]), r++)
          : (i && i(),
            t &&
              (0 === a.length &&
                (Graphics$1.printLoadingError(t), SceneManager.stop()),
              a.push(() => {
                (r = 0), e();
              })));
      };
  ...
  class Graphics {
    ...
    static printErrorDetail(t) {
      if (this._errorPrinter && this._showErrorDetail) {
        const e = this._formatEventInfo(t),
          ...
          s = i ? `${e}, ${i}` : e,
          a = this._formatStackTrace(t);
        this._makeErrorDetail(s, a);
      }
    }
    ...
    static loadFont(t, e) {
      const i = document.createElement("style"),
        ...
        a = `@font-face { font-family: "${t}"; src: url("${e}"); }`;
      (i.type = "text/css"),
        s.item(0).appendChild(i),
        i.sheet.insertRule(a, 0),
    ...
    static setVideoVolume(t) {
      (this._videoVolume = t),
        this._video && (this._video.volume = this._videoVolume);
    }
    ...
    static _testCanvasBlendModes() {
      ...
      (t = document.createElement("canvas")),
        ...
        (e.fillStyle = "white"),
        e.fillRect(0, 0, 1, 1),
        (s = e.getImageData(0, 0, 1, 1)),
        (this._canUseDifferenceBlend = 0 === i.data[0]),
        (this._canUseSaturationBlend = 0 === s.data[0]);
    }
    ...
    static _paintUpperCanvas() {
      if (
        ...
        const t = this._upperCanvas.getContext("2d"),
          ...
          s = ((this._loadingCount - 20) / 30).clamp(0, 1);
        t.save(),
          (t.globalAlpha = s),
    ...
    static _updateVisibility(t) {
      (this._video.style.opacity = t ? 1 : 0),
        (this._canvas.style.opacity = t ? 0 : 1);
    }
    ...
    static _onTouchEnd(t) {
      this._videoUnlocked || (this._video.play(), (this._videoUnlocked = !0)),
        this._isVideoVisible() && this._video.paused && this._video.play();
    }
    ...
    static _createPixiApp() {
      ...
      try {
        (this._app = new PIXI.Application(t)),
  ...
  class Rectangle extends PIXI.Rectangle {
    ...
    initialize(t, e, i, s) {
      PIXI.Rectangle.call(this, t, e, i, s);
    }
  }
  ...
  class Bitmap {
    ...
    _createBaseTexture(t) {
      (this.__baseTexture = new PIXI.BaseTexture(t)),
    ...
    blt({ width: t, height: e, _canvas: i }, s, a, r, n, h, o, l, c) {
      (l = l || r),
        (c = c || n),
        (s = Math.floor(s)),
        (a = Math.floor(a)),
        ...
        s >= 0 &&
          ...
          this._context.drawImage(i, s, a, r, n, h, o, l, c),
          this._setDirty());
    }
    bltImage({ width: t, height: e, _image: i }, s, a, r, n, h, o, l, c) {
      (l = l || r),
        (c = c || n),
        s >= 0 &&
          a >= 0 &&
    ...
    clearRect(t, e, i, s) {
      this._context.clearRect(t, e, i, s), this._setDirty();
    }
    clear() {
    ...
    gradientFillRect(t, e, i, s, a, r, n) {
      const h = this._context;
      let o;
      (o = n
        ? h.createLinearGradient(t, e, t, e + s)
        : h.createLinearGradient(t, e, t + i, e)),
        o.addColorStop(0, a),
        o.addColorStop(1, r),
        h.save(),
        (h.fillStyle = o),
    ...
    drawCircle(t, e, i, s) {
      ...
      a.save(),
        (a.fillStyle = s),
        a.beginPath(),
        a.arc(t, e, i, 0, 2 * Math.PI, !1),
        a.fill(),
        a.restore(),
        this._setDirty();
    }
    drawText(t, e, i, s, a, r) {
      if (void 0 !== t) {
        (e = Math.floor(e)),
          (i = Math.floor(i)),
          (s = Math.floor(s) || 4294967295);
        let n = e;
        const h =
            i + (a = Math.floor(a)) - Math.round((a - 0.7 * this.fontSize) / 2),
          o = this._context,
          l = o.globalAlpha;
        "center" === r && (n += s / 2),
          "right" === r && (n += s),
          o.save(),
          (o.font = this._makeFontNameText()),
          (o.textAlign = r),
          (o.textBaseline = "alphabetic"),
          (o.globalAlpha = 1),
          this._drawTextOutline(t, n, h, s),
          (o.globalAlpha = l),
          this._drawTextBody(t, n, h, s),
          o.restore(),
          this._setDirty();
      }
    }
    drawSmallText(t, e, i, s, a, r) {}
    measureTextWidth(t) {
    ...
    adjustTone(t, e, i) {
      if ((t || e || i) && this.width > 0 && this.height > 0) {
        const s = this._context,
          a = s.getImageData(0, 0, this.width, this.height),
          r = a.data;
        for (let s = 0; s < r.length; s += 4)
          (r[s + 0] += t), (r[s + 1] += e), (r[s + 2] += i);
        s.putImageData(a, 0, 0), this._setDirty();
      }
    }
    rotateHue(t) {
      if (t && t && this.width > 0 && this.height > 0) {
        ...
        const s = this._context,
          ...
          r = a.data;
        for (let s = 0; s < r.length; s += 4) {
          const a = e(r[s + 0], r[s + 1], r[s + 2]),
            n = i((a[0] + t) % 360, a[1], a[2]);
          (r[s + 0] = n[0]), (r[s + 1] = n[1]), (r[s + 2] = n[2]);
        }
        s.putImageData(a, 0, 0), this._setDirty();
      }
      function e(t, e, i) {
          ...
          o > 0 &&
            ((r =
              t === a
                ...
                ? 60 * ((i - t) / o + 2)
                : 60 * ((t - e) / o + 4)),
            (n = o / (255 - Math.abs(2 * h - 255)))),
          [r, n, h]
        );
      }
      function i(t, e, i) {
        const s = (255 - Math.abs(2 * i - 255)) * e,
          a = i - s / 2,
          r = s + a,
          n = s * (1 - Math.abs(((t / 60) % 2) - 1)) + a;
        return t < 60
          ? [r, n, a]
          : t < 120
          ? [n, r, a]
    ...
    blur() {
      for (let t = 0; t < 2; t++) {
        ...
        (a.width = t + 2),
          (a.height = e + 2),
          r.drawImage(i, 0, 0, t, e, 1, 1, t, e),
          r.drawImage(i, 0, 0, t, 1, 1, 0, t, 1),
          r.drawImage(i, 0, 0, 1, e, 0, 1, 1, e),
          r.drawImage(i, 0, e - 1, t, 1, 1, e + 1, t, 1),
          r.drawImage(i, t - 1, 0, 1, e, t + 1, 1, 1, e),
          s.save(),
          (s.fillStyle = "black"),
          s.fillRect(0, 0, t, e),
          ...
          (s.globalAlpha = 1 / 9);
        for (let i = 0; i < 3; i++)
          for (let r = 0; r < 3; r++) s.drawImage(a, r, i, t, e, 0, 0, t, e);
        s.restore();
      }
      this._setDirty();
    }
    ...
    _drawTextOutline(t, e, i, s) {
      const a = this._context;
      (a.strokeStyle = this.outlineColor),
        (a.lineWidth = this.outlineWidth),
        (a.lineJoin = "round"),
        a.strokeText(t, e, i, s);
    }
    _drawTextBody(t, e, i, s) {
      const a = this._context;
      (a.fillStyle = this.textColor), a.fillText(t, e, i, s);
    }
    _onLoad() {
      switch (
    ...
    static snap(t) {
      const e = Graphics$1.width,
        i = Graphics$1.height,
        s = new Bitmap(e, i),
        a = s._context,
        r = PIXI.RenderTexture.create({ width: e, height: i });
      if (t) {
        Graphics$1._renderer.render(t, { renderTexture: r }),
          t.worldTransform.identity();
        let e = null;
        (e = Graphics$1.isWebGL()
          ? Graphics$1._renderer.plugins.extract.canvas(r)
          : r.baseTexture._canvasRenderTarget.canvas),
          a.drawImage(e, 0, 0);
      }
      return r.destroy({ destroyBase: !0 }), s._setDirty(), s;
    }
    static request(t) {
      const e = Object.create(Bitmap.prototype);
      return (
  ...
  class ImageManager {
    ...
    static loadBitmap(t, e, i, s) {
      if (e) {
        const s = `${t + encodeURIComponent(e)}.png`,
          a = this.loadNormalBitmap(s, i || 0);
        return (a.smooth = !1), a;
      }
    ...
    static loadNormalBitmap(t, e) {
      ...
      let s = this._imageCache.get(i);
      return (
        s
          ? s.isReady() || s.decode()
          : ((s = Bitmap.load(t)),
            this._callCreationHook(s),
            s.addLoadListener(() => {
              s.rotateHue(e);
            }),
            this._imageCache.add(i, s)),
        s
      );
    }
    static clear() {
    ...
    static reserveBitmap(t, e, i, s, a) {
      if (e) {
        const s = `${t + encodeURIComponent(e)}.png`,
          r = this.reserveNormalBitmap(
            s,
            i || 0,
            a || this._defaultReservationId
          );
        return (r.smooth = !1), r;
      }
      return this.loadEmptyBitmap();
    ...
    static reserveNormalBitmap(t, e, i) {
      const s = this.loadNormalBitmap(t, e);
      return this._imageCache.reserve(this._generateCacheKey(t, e), s, i), s;
    }
    ...
    static requestBitmap(t, e, i, s) {
      if (e) {
        const s = `${t + encodeURIComponent(e)}.png`,
          a = this.requestNormalBitmap(s, i || 0);
        return (a.smooth = !1), a;
      }
      return this.loadEmptyBitmap();
    }
      ...
      return (
        s
          ...
          : ((s = Bitmap.request(t)),
            ...
            this._imageCache.add(i, s),
            this._requestQueue.enqueue(i, s)),
        s
      );
    }
    static update() {
  ...
  class BitmapPIXI extends PIXI.Container {
    ...
    clearRect(t, e, i, s) {
      ...
      this.children.forEach((a) => {
        a &&
          ...
          a.y >= e &&
          a.y < e + s &&
          (a.isBitmapText ? (a.text = "") : r.push(a));
      }),
        r.forEach((t) => {
          t.destroy({ children: !0, texture: !0 }), a.removeChild(t);
        });
    }
    drawText(t, e, i, s, a, r) {
      if (void 0 === t) return;
      const n = this._paintOpacity / 255;
      (s = Math.floor(s) || 4294967295),
        (a = Math.floor(a)),
        (t = String(t)),
        "center" === r ? (e += s / 2) : "right" === r && (e += s),
        (i = i + a - 1.25 * this.fontSize),
        (e = Math.floor(e)),
        (i = Math.floor(i)),
        this._updateExistingText(t, e, i, n) ||
          this._drawNewText(t, e, i, n, s, a, r);
    }
    _updateExistingText(t, e, i, s) {
      for (const a of this.textCache)
        if (a.x === e && a.y === i) {
          const e = PIXI.utils.string2hex(this.textColor);
          return (
            a._tint !== e && (a.tint = e),
            a.text !== t && (a.text = t),
            a.alpha !== s && (a.alpha = s),
    ...
    _drawNewText(t, e, i, s, a, r, n) {
      ...
      PIXI.BitmapFont.available[h.fontFamily] || this._makeBitmapFont(h);
      const o = new PIXI.BitmapText(t, {
        fontName: h.fontFamily,
        fontSize: h.fontSize,
      });
      !h.wordWrap && o.width > a && (o.scale.x = a / o.width),
        "center" === n
          ? o.anchor.set(0.5, 0)
          : "right" === n && o.anchor.set(1, 0),
        o &&
          ((o.x = e),
          (o.y = i),
          (o.tint = PIXI.utils.string2hex(this.textColor)),
          (o.alpha = s),
          (o.isBitmapText = !0),
          this.textCache.push(o),
          this.addChild(o));
    }
    _makeBitmapFont(t) {
      PIXI.BitmapFont.from(t.fontFamily, t, { chars: [[" ", "~"], "→", "’"] });
    }
    measureTextWidth(t) {
      t = String(t);
      ...
      return PIXI.TextMetrics.measureText(t, e).width;
    }
    create9Slice(t, e, i, s, a, r, n, h, o) {
      return new PIXI.NineSlicePlane(
        new PIXI.Texture(t, new PIXI.Rectangle(e, i, s, a)),
        r,
        n,
        h,
        o
    ...
    createTilingSprite(t, e, i, s, a, r, n) {
    ...
    createCroppedSprite(t, e, i, s, a) {
      return new PIXI.Sprite(
        new PIXI.Texture(t, new PIXI.Rectangle(e, i, s, a))
      );
    }
    blt({ width: t, height: e, baseTexture: i }, s, a, r, n, h, o, l, c) {
      if (
        ((l = l || r),
        (c = c || n),
        (s = Math.floor(s)),
        ...
        const t = this.createCroppedSprite(i, s, a, r, n);
        if (t)
          return (
            (t.x = h),
            (t.y = o),
    ...
    fillRect(t, e, i, s, a) {
      ...
      return (
        ...
        r &&
          ((r.x = t),
          (r.y = e),
          (r.alpha = this._paintOpacity / 255),
          this.addChild(r)),
        r
      );
    }
    gradientFillRect(t, e, i, s, a, r, n) {
      return this.fillRect(t, e, i, s, a);
    }
    drawCircle(t, e, i, s) {
      (t = Math.floor(t)), (e = Math.floor(e));
      const a = new PIXI.Graphics();
      return (
        (s = PIXI.utils.string2hex(s)),
        a.beginFill(s),
        a.drawCircle(0, 0, i),
        a.endFill(),
        a &&
          ((a.x = t),
          (a.y = e),
          (a.alpha = this._paintOpacity / 255),
          this.addChild(a)),
        a
      );
    }
  ...
  class Sprite extends PIXI.Sprite {
    ...
    initialize(t) {
      const e = new PIXI.Texture(new PIXI.BaseTexture());
    ...
    setFrame(t, e, i, s) {
      this._refreshFrame = !1;
      const a = this._frame;
      (t === a.x && e === a.y && i === a.width && s === a.height) ||
        ((a.x = t), (a.y = e), (a.width = i), (a.height = s), this._refresh());
    }
    getBlendColor() {
    ...
    _refresh() {
      const t = Math.floor(this._frame.x),
        ...
        s = Math.floor(this._frame.height),
        a = this._bitmap ? this._bitmap.width : 0,
        r = this._bitmap ? this._bitmap.height : 0,
        n = t.clamp(0, a),
        h = e.clamp(0, r),
        o = (i - n + t).clamp(0, a - n),
        l = (s - h + e).clamp(0, r - h);
      (this._realFrame.x = n),
        (this._realFrame.y = h),
        (this._realFrame.width = o),
        (this._realFrame.height = l),
        (this.pivot.x = t - n),
        (this.pivot.y = e - h),
        o > 0 && l > 0
          ? this._needsTint()
            ? Graphics$1.isWebGL()
              ...
              : (this._createTinter(o, l),
                this._executeTint(n, h, o, l),
                this._tintTexture.update(),
                (this.texture.baseTexture = this._tintTexture),
                (this.texture.frame = new Rectangle(0, 0, o, l)))
            : (this._bitmap &&
    ...
    _executeTint(t, e, i, s) {
      const a = this._context,
        r = this._colorTone,
        n = this._blendColor;
      if (
        ((a.globalCompositeOperation = "copy"),
        a.drawImage(this._bitmap.canvas, t, e, i, s, 0, 0, i, s),
        r[0] || r[1] || r[2] || r[3])
      ) {
        if (Graphics$1.canUseSaturationBlend()) {
          const t = Math.max(0, r[3]);
          (a.globalCompositeOperation = "saturation"),
            (a.fillStyle = `rgba(255,255,255,${t / 255})`),
            a.fillRect(0, 0, i, s);
        }
        ...
        if (
          ...
          (a.fillStyle = Utils.rgbToCssColor(t, e, n)),
          a.fillRect(0, 0, i, s),
          Graphics$1.canUseDifferenceBlend())
        ) {
      ...
      const h = Math.max(0, n[0]),
        o = Math.max(0, n[1]),
        l = Math.max(0, n[2]),
        c = Math.max(0, n[3]);
      (a.globalCompositeOperation = "source-atop"),
        (a.fillStyle = Utils.rgbToCssColor(h, o, l)),
        (a.globalAlpha = c / 255),
        a.fillRect(0, 0, i, s),
    ...
    _executeTintWebGL() {
      ...
      (this._colorMatrixFilter.matrix = [
        ...
        (this._colorMatrixFilter.alpha = a);
    }
    _clearTintWebGL() {
  ...
  class Tilemap extends PIXI.Container {
    ...
    initialize() {
      PIXI.Container.call(this),
        (this._margin = 20),
        (this._width = Graphics$1.width + 2 * this._margin),
        (this._height = Graphics$1.height + 2 * this._margin),
        (this._tileWidth = 48),
        (this._tileHeight = 48),
        (this._mapWidth = 0),
        (this._mapHeight = 0),
    ...
    updateTransform() {
      const t = Math.floor(this.origin.x),
        ...
        s = Math.floor((e - this._margin) / this._tileHeight);
      this._updateLayerPositions(i, s),
        (this._needsRepaint ||
    ...
    _createLayers() {
      const t = this._width,
        e = this._height,
        i = this._margin,
        s = Math.ceil(t / this._tileWidth) + 1,
        a = Math.ceil(e / this._tileHeight) + 1,
        r = s * this._tileWidth,
        n = a * this._tileHeight;
      (this._lowerBitmap = new Bitmap(r, n)),
        (this._upperBitmap = new Bitmap(r, n)),
        (this._layerWidth = r),
        (this._layerHeight = n),
    ...
    _updateLayerPositions(t, e) {
      const i = this._margin,
        s = Math.floor(this.origin.x),
        a = Math.floor(this.origin.y),
        r = (s - i).mod(this._layerWidth),
        n = (a - i).mod(this._layerHeight),
        h = this._layerWidth - r,
        o = this._layerHeight - n,
        l = this._width - h,
        c = this._height - o;
      for (let t = 0; t < 2; t++) {
        let e;
        (e = 0 === t ? this._lowerLayer.children : this._upperLayer.children),
          e[0].move(0, 0, h, o),
          e[0].setFrame(r, n, h, o),
          e[1].move(h, 0, l, o),
          e[1].setFrame(0, n, l, o),
          e[2].move(0, o, h, c),
          e[2].setFrame(r, 0, h, c),
          e[3].move(h, o, l, c),
          e[3].setFrame(0, 0, l, c);
      }
    }
    _paintAllTiles(t, e) {
      const i = Math.ceil(this._width / this._tileWidth) + 1,
        s = Math.ceil(this._height / this._tileHeight) + 1;
      for (let a = 0; a < s; a++)
        for (let s = 0; s < i; s++) this._paintTiles(t, e, s, a);
    }
    _paintTiles(t, e, i, s) {
      const a = t + i,
        r = e + s,
        n = (a * this._tileWidth).mod(this._layerWidth),
        h = (r * this._tileHeight).mod(this._layerHeight),
        o = n / this._tileWidth,
        l = h / this._tileHeight,
        c = this._readMapData(a, r, 0),
        d = this._readMapData(a, r, 1),
        m = this._readMapData(a, r, 2),
        u = this._readMapData(a, r, 3),
        p = this._readMapData(a, r, 4),
        _ = this._readMapData(a, r - 1, 1),
        g = [],
        f = [];
      this._isHigherTile(c) ? f.push(c) : g.push(c),
        this._isHigherTile(d) ? f.push(d) : g.push(d),
        g.push(-p),
        this._isTableTile(_) &&
          !this._isTableTile(d) &&
          (Tilemap.isShadowingTile(c) || g.push(1e4 + _)),
        this._isOverpassPosition(a, r)
          ? (f.push(m), f.push(u))
          : (this._isHigherTile(m) ? f.push(m) : g.push(m),
            this._isHigherTile(u) ? f.push(u) : g.push(u));
      const w = this._readLastTiles(0, o, l);
      if (!g.equals(w) || (Tilemap.isTileA1(c) && this._frameUpdated)) {
        this._lowerBitmap.clearRect(n, h, this._tileWidth, this._tileHeight);
        for (const t of g)
          t < 0
            ? this._drawShadow(this._lowerBitmap, p, n, h)
            : t >= 1e4
            ? this._drawTableEdge(this._lowerBitmap, _, n, h)
            : this._drawTile(this._lowerBitmap, t, n, h);
        this._writeLastTiles(0, o, l, g);
      }
      const y = this._readLastTiles(1, o, l);
      if (!f.equals(y)) {
        this._upperBitmap.clearRect(n, h, this._tileWidth, this._tileHeight);
        for (let t = 0; t < f.length; t++)
          this._drawTile(this._upperBitmap, f[t], n, h);
        this._writeLastTiles(1, o, l, f);
      }
    }
    _readLastTiles(t, e, i) {
      const s = this._lastTiles[t];
      if (s) {
        const t = s[i];
        if (t) {
          const i = t[e];
    ...
    _writeLastTiles(t, e, i, s) {
      let a = this._lastTiles[t];
      a || (a = this._lastTiles[t] = []);
      let r = a[i];
      r || (r = a[i] = []), (r[e] = s);
    }
    _drawTile(t, e, i, s) {
      Tilemap.isVisibleTile(e) &&
        (Tilemap.isAutotile(e)
          ? this._drawAutotile(t, e, i, s)
          : this._drawNormalTile(t, e, i, s));
    }
    _drawNormalTile(t, e, i, s) {
      let a = 0;
      a = Tilemap.isTileA5(e) ? 4 : 5 + Math.floor(e / 256);
      const r = this._tileWidth,
        n = this._tileHeight,
        h = ((Math.floor(e / 128) % 2) * 8 + (e % 8)) * r,
        o = (Math.floor((e % 256) / 8) % 16) * n,
        l = this.bitmaps[a];
      l && t.bltImage(l, h, o, r, n, i, s, r, n);
    }
    _drawAutotile(t, e, i, s) {
      let a = Tilemap.FLOOR_AUTOTILE_TABLE;
      const r = Tilemap.getAutotileKind(e),
        n = Tilemap.getAutotileShape(e),
        h = r % 8,
        o = Math.floor(r / 8);
      let l = 0,
        c = 0,
      ...
      if (Tilemap.isTileA1(e)) {
        ...
        (d = 0),
          0 === r
            ...
            ? ((l = 6), (c = 3))
            : ((l = 8 * Math.floor(h / 4)),
              (c = 6 * o + (Math.floor(h / 2) % 2) * 3),
              r % 2 == 0
                ? (l += 2 * t)
                : ((l += 6),
                  (a = Tilemap.WATERFALL_AUTOTILE_TABLE),
                  (c += this.animationFrame % 3)));
      } else
        Tilemap.isTileA2(e)
          ? ((d = 1),
            (l = 2 * h),
            (c = 3 * (o - 2)),
            (m = this._isTableTile(e)))
          : Tilemap.isTileA3(e)
          ? ((d = 2),
            (l = 2 * h),
            (c = 2 * (o - 6)),
            (a = Tilemap.WALL_AUTOTILE_TABLE))
          : Tilemap.isTileA4(e) &&
            ((d = 3),
            (l = 2 * h),
            (c = Math.floor(2.5 * (o - 10) + (o % 2 == 1 ? 0.5 : 0))),
            o % 2 == 1 && (a = Tilemap.WALL_AUTOTILE_TABLE));
      const u = a[n],
        p = this.bitmaps[d];
      if (u && p) {
        const e = this._tileWidth / 2,
          a = this._tileHeight / 2;
        for (let r = 0; r < 4; r++) {
          const n = u[r][0],
            h = u[r][1],
            o = (2 * l + n) * e,
            d = (2 * c + h) * a,
            _ = i + (r % 2) * e;
          let g = s + Math.floor(r / 2) * a;
          if (!m || (1 !== h && 5 !== h)) t.bltImage(p, o, d, e, a, _, g, e, a);
          else {
            let i = n;
            1 === h && (i = [0, 3, 2, 1][n]);
            const s = (2 * l + i) * e,
              r = (2 * c + 3) * a;
            t.bltImage(p, s, r, e, a, _, g, e, a),
              (g += a / 2),
              t.bltImage(p, o, d, e, a / 2, _, g, e, a / 2);
          }
        }
      }
    }
    _drawTableEdge(t, e, i, s) {
      if (Tilemap.isTileA2(e)) {
        const a = Tilemap.FLOOR_AUTOTILE_TABLE,
          r = Tilemap.getAutotileKind(e),
          n = Tilemap.getAutotileShape(e),
          h = 1,
          o = (r % 8) * 2,
          l = 3 * (Math.floor(r / 8) - 2),
          c = a[n];
        if (c) {
          const e = this.bitmaps[h],
            a = this._tileWidth / 2,
            r = this._tileHeight / 2;
          for (let n = 0; n < 2; n++) {
            const h = (2 * o + c[2 + n][0]) * a,
              d = (2 * l + c[2 + n][1]) * r + r / 2,
              m = i + (n % 2) * a,
              u = s + Math.floor(n / 2) * r;
            t.bltImage(e, h, d, a, r / 2, m, u, a, r / 2);
          }
        }
      }
    }
    _drawShadow(t, e, i, s) {
      if (15 & e) {
        const a = this._tileWidth / 2,
          r = this._tileHeight / 2,
          n = "rgba(0,0,0,0.5)";
        for (let h = 0; h < 4; h++)
          if (e & (1 << h)) {
            const e = i + (h % 2) * a,
              o = s + Math.floor(h / 2) * r;
            t.fillRect(e, o, a, r, n);
          }
      }
    }
    _readMapData(t, e, i) {
      if (this._mapData) {
        const s = this._mapWidth,
          a = this._mapHeight;
        return (
          this.horizontalWrap && (t = t.mod(s)),
          this.verticalWrap && (e = e.mod(a)),
          (t >= 0 &&
            t < s &&
            e >= 0 &&
            e < a &&
            this._mapData[(i * a + e) * s + t]) ||
            0
        );
      }
      return 0;
  ...
  class ShaderTilemap extends Tilemap {
    ...
    renderCanvas(t) {
      this._hackRenderer(t),
        PIXI.Container.prototype.renderCanvas.call(this, t);
    }
    render(t) {
      this._hackRenderer(t), PIXI.Container.prototype.render.call(this, t);
    }
    refresh() {
    ...
    refreshTileset() {
      const t = this.bitmaps.map((t) =>
        t._baseTexture ? new PIXI.Texture(t._baseTexture) : t
      );
    ...
    _updateLayerPositions(t, e) {
      ...
      (this.lowerZLayer.position.x = t * this._tileWidth - i),
        (this.lowerZLayer.position.y = e * this._tileHeight - s),
        (this.upperZLayer.position.x = t * this._tileWidth - i),
        (this.upperZLayer.position.y = e * this._tileHeight - s);
    }
    _paintAllTiles(t, e) {
    ...
    _paintTiles(t, e, i, s) {
      const a = t + i,
        r = e + s,
        n = i * this._tileWidth,
        h = s * this._tileHeight,
        o = this._readMapData(a, r, 0),
        l = this._readMapData(a, r, 1),
        c = this._readMapData(a, r, 2),
        d = this._readMapData(a, r, 3),
        m = this._readMapData(a, r, 4),
        u = this._readMapData(a, r - 1, 1),
        p = this.lowerLayer.children[0],
        _ = this.upperLayer.children[0];
      this._isHigherTile(o)
        ? this._drawTile(_, o, n, h)
        : this._drawTile(p, o, n, h),
        this._isHigherTile(l)
          ? this._drawTile(_, l, n, h)
          : this._drawTile(p, l, n, h),
        this._drawShadow(p, m, n, h),
        this._isTableTile(u) &&
          !this._isTableTile(l) &&
          (Tilemap.isShadowingTile(o) || this._drawTableEdge(p, u, n, h)),
        this._isOverpassPosition(a, r)
          ? (this._drawTile(_, c, n, h), this._drawTile(_, d, n, h))
          : (this._isHigherTile(c)
              ? this._drawTile(_, c, n, h)
              : this._drawTile(p, c, n, h),
            this._isHigherTile(d)
              ? this._drawTile(_, d, n, h)
              : this._drawTile(p, d, n, h));
    }
    _drawTile(t, e, i, s) {
      Tilemap.isVisibleTile(e) &&
        (Tilemap.isAutotile(e)
    ...
    _drawAutotile(t, e, i, s) {
        ...
        ? ((d = 0),
          0 === r
            ...
            : ((l = 8 * Math.floor(h / 4)),
              ...
              r % 2 == 0
                ? (u = 2)
                : ((l += 6), (a = Tilemap.WATERFALL_AUTOTILE_TABLE), (p = 1))))
        : Tilemap.isTileA2(e)
        ? ((d = 1), (l = 2 * h), (c = 3 * (o - 2)), (m = this._isTableTile(e)))
        : Tilemap.isTileA3(e)
        ? ((d = 2),
          (l = 2 * h),
          (c = 2 * (o - 6)),
      ...
      for (let e = 0; e < 4; e++) {
        const a = _[e][0],
          ...
          h = (2 * c + r) * f,
          o = i + (e % 2) * g,
          w = s + Math.floor(e / 2) * f;
        if (!m || (1 !== r && 5 !== r)) t.addRect(d, n, h, o, w, g, f, u, p);
        else {
          let e = a;
          1 === r && (e = (4 - a) % 4);
          const i = (2 * l + e) * g,
            s = (2 * c + 3) * f;
          t.addRect(d, i, s, o, w, g, f, u, p),
            t.addRect(d, n, h, o, w + f / 2, g, f / 2, u, p);
        }
      }
    }
    _drawTableEdge(t, e, i, s) {
      if (Tilemap.isTileA2(e)) {
        const a = Tilemap.FLOOR_AUTOTILE_TABLE,
          ...
          c = a[n],
          d = this._tileWidth / 2,
          m = this._tileHeight / 2;
        for (let e = 0; e < 2; e++) {
          const a = (2 * o + c[2 + e][0]) * d,
            r = (2 * l + c[2 + e][1]) * m + m / 2,
            n = i + (e % 2) * d,
            u = s + Math.floor(e / 2) * m;
          t.addRect(h, a, r, n, u, d, m / 2);
        }
      }
    }
    _drawShadow(t, e, i, s) {
      if (15 & e) {
        ...
        for (let n = 0; n < 4; n++)
          if (e & (1 << n)) {
            const e = i + (n % 2) * a,
              h = s + Math.floor(n / 2) * r;
            t.addRect(-1, 0, 0, e, h, a, r);
          }
      }
    }
  }
  (PIXI.tilemap.Constant.SCALE_MODE = PIXI.SCALE_MODES.NEAREST),
    (PIXI.tilemap.Constant.DO_CLEAR = !0),
  ...
  class Window extends PIXI.Container {
    ...
    move(t, e, i, s) {
      (this.x = Math.floor(t || 0)),
        ...
        (this._width === i && this._height === s) ||
          ((this._width = Math.floor(i || 0)),
          (this._height = Math.floor(s || 0)),
    ...
    setCursorRect(t, e, i, s) {
      const a = Math.floor(t || 0),
        r = Math.floor(e || 0),
        n = Math.floor(i || 0),
        h = Math.floor(s || 0),
        o = this._cursorRect;
      (o.x === a && o.y === r && o.width === n && o.height === h) ||
        ((this._cursorRect.x = a),
        (this._cursorRect.y = r),
        (this._cursorRect.width = n),
        (this._cursorRect.height = h),
        this._refreshCursor());
    }
    setTone(t, e, i) {
      const s = this._colorTone;
      (t /= 255) < 0 && (t = 0),
        (e /= 255) < 0 && (e = 0),
        (i /= 255) < 0 && (i = 0),
        (t === s[0] && e === s[1] && i === s[2]) ||
    ...
    _refreshBack() {
      const t = this._margin,
        e = this._width - 2 * t,
        i = this._height - 2 * t,
        s = PIXI.utils.rgb2hex(this._colorTone);
      if (
        e > 0 &&
        i > 0 &&
        this._windowskin &&
      ...
      (this._windowBackSprite.width = e),
        ...
        this._windowBackSprite.children.forEach((t) => {
          t && ((t.width = e), (t.height = i), (t.tint = s));
        });
    }
    _refreshFrame() {
      const t = this._width,
      ...
      if (
        ...
        let s;
        const a = WindowSkinCache.getItem(this._windowskin._url, "frame");
        if (a) s = a;
        else {
          const a = new BitmapPIXI(),
            r = this._windowskin,
            n = 96,
            h = 96;
          a.blt(r, n + i, 0, n - 48, i, i, 0, t - 48, i),
            a.blt(r, n + i, 0 + h - i, n - 48, i, i, e - i, t - 48, i),
            a.blt(r, n + 0, 24, i, n - 48, 0, i, i, e - 48),
            a.blt(r, n + h - i, 24, i, n - 48, t - i, i, i, e - 48),
            a.blt(r, n + 0, 0, i, i, 0, 0, i, i),
            a.blt(r, n + h - i, 0, i, i, t - i, 0, i, i),
            a.blt(r, n + 0, 0 + h - i, i, i, 0, e - i, i, i),
            a.blt(r, n + h - i, 0 + h - i, i, i, t - i, e - i, i, i),
            (s = Graphics$1._renderer.generateTexture(a)),
            a.destroy({ children: !0, texture: !0 }),
            WindowSkinCache.setItem(this._windowskin._url, s, "frame");
        }
        (this._windowFramePlane = new PIXI.NineSlicePlane(s, 12, 12, 12, 12)),
    ...
    _refreshCursor() {
      const t = this._padding,
        ...
        s = this._cursorRect.width,
        a = this._cursorRect.height;
      if (
        s > 0 &&
        a > 0 &&
        this._windowskin &&
      ...
      this._windowCursorPlane &&
        ...
        (this._windowCursorPlane.width = s),
        (this._windowCursorPlane.height = a));
    }
    _refreshContents() {
  ...
  (WindowLayer.voidFilter = new PIXI.filters.AlphaFilter()),
    ...
    (WindowLayer.prototype.renderCanvas =
      PIXI.Container.prototype.renderCanvas);
  class Weather extends PIXI.Container {
    ...
    _rebornSprite(t) {
      (t.ax = Math.randomInt(Graphics$1.width + 100) - 100 + this.origin.x),
        (t.ay = Math.randomInt(Graphics$1.height + 200) - 200 + this.origin.y),
  ...
  class ToneFilter extends PIXI.filters.ColorMatrixFilter {
    ...
    adjustTone(t, e, i) {
      if (
        ...
        const s = [1, 0, 0, t, 0, 0, 1, 0, e, 0, 0, 0, 1, i, 0, 0, 0, 0, 1, 0];
  ...
  class ToneSprite extends PIXI.Container {
    ...
    setTone(t, e, i, s) {
      (this._red = Math.round(t || 0).clamp(-255, 255)),
        ...
        (this._gray = Math.round(s || 0).clamp(0, 255));
    }
    _renderCanvas(t) {
      if (this.visible) {
        const e = t.context,
          ...
          r = Graphics$1.height;
        e.save(),
          e.setTransform(i.a, i.b, i.c, i.d, i.tx * s, i.ty * s),
          Graphics$1.canUseSaturationBlend() &&
            this._gray > 0 &&
            ((e.globalCompositeOperation = "saturation"),
            ...
            e.fillRect(0, 0, a, r)),
          (e.globalAlpha = 1);
        const n = Math.max(0, this._red),
          h = Math.max(0, this._green),
          o = Math.max(0, this._blue);
        if (
          ((n || h || o) &&
            ((e.globalCompositeOperation = "lighter"),
            (e.fillStyle = Utils.rgbToCssColor(n, h, o)),
            e.fillRect(0, 0, a, r)),
          Graphics$1.canUseDifferenceBlend())
        ) {
          const t = Math.max(0, -this._red),
            i = Math.max(0, -this._green),
            s = Math.max(0, -this._blue);
          (t || i || s) &&
            ((e.globalCompositeOperation = "difference"),
            (e.fillStyle = "#ffffff"),
            e.fillRect(0, 0, a, r),
            (e.globalCompositeOperation = "lighter"),
            (e.fillStyle = Utils.rgbToCssColor(t, i, s)),
            e.fillRect(0, 0, a, r),
            (e.globalCompositeOperation = "difference"),
            (e.fillStyle = "#ffffff"),
            e.fillRect(0, 0, a, r));
  ...
  class Stage extends PIXI.Container {
    ...
    initialize() {
      PIXI.Container.call(this), (this.interactive = !1);
    }
  }
  class JsonEx {
    constructor() {
    ...
    static parse(t) {
      const e = [],
        i = {},
        s = this._decode(JSON.parse(t), e, i);
      return this._cleanMetadata(s), this._linkCircularReference(s, e, i), s;
    }
    ...
    static _encode(t, e, i = 0) {
      ...
      if ("[object Object]" === s || "[object Array]" === s) {
        t["@c"] = JsonEx._generateId();
        const s = this._getConstructorName(t);
        "Object" !== s && "Array" !== s && (t["@"] = s);
        for (let s in t)
          (t.hasOwnProperty && !t.hasOwnProperty(s)) ||
            s.match(/^@./) ||
            (t[s] && "object" == typeof t[s]
              ? t[s]["@c"]
                ? (e.push([s, t, t[s]]), (t[s] = { "@r": t[s]["@c"] }))
                : ((t[s] = this._encode(t[s], e, i + 1)),
      ...
      if ("[object Object]" === s || "[object Array]" === s) {
        ...
        for (let s in t)
          if (!t.hasOwnProperty || t.hasOwnProperty(s)) {
            ...
            t[s] && t[s]["@r"] && e.push([s, t, t[s]["@r"]]),
              (t[s] = this._decode(t[s], e, i));
          }
      }
      return t;
    }
  ...
  class StorageManager {
    ...
    static save(t, e) {
      this.isLocalMode()
        ? this.saveToLocalFile(t, e)
        : this.saveToWebStorage(t, e);
    }
    static load(t) {
      return this.isLocalMode()
        ? this.loadFromLocalFile(t)
        : this.loadFromWebStorage(t);
    }
    static exists(t) {
      return this.isLocalMode()
        ? this.localFileExists(t)
        : this.webStorageExists(t);
    }
    static remove(t) {
    ...
    static backup(t) {
      if (this.exists(t))
        if (this.isLocalMode()) {
          const e = this.loadFromLocalFile(t),
            i = LZString.compressToBase64(e),
            s = require("fs"),
            a = this.localFileDirectoryPath(),
            r = `${this.localFilePath(t)}.bak`;
          s.existsSync(a) || s.mkdirSync(a), s.writeFileSync(r, i);
        } else {
          const e = this.loadFromWebStorage(t),
            i = LZString.compressToBase64(e),
            s = `${this.webStorageKey(t)}bak`;
          localStorage.setItem(s, i);
        }
    }
    static backupExists(t) {
      return this.isLocalMode()
        ? this.localFileBackupExists(t)
        : this.webStorageBackupExists(t);
    }
    static cleanBackup(t) {
      if (this.backupExists(t))
        if (this.isLocalMode()) {
          const e = require("fs"),
            i = this.localFilePath(t);
          e.unlinkSync(`${i}.bak`);
        } else {
          const e = this.webStorageKey(t);
          localStorage.removeItem(`${e}bak`);
        }
    }
    static restoreBackup(t) {
      if (this.backupExists(t))
        if (this.isLocalMode()) {
          const e = this.loadFromLocalBackupFile(t),
            i = LZString.compressToBase64(e),
            s = require("fs"),
            a = this.localFileDirectoryPath(),
            r = this.localFilePath(t);
          s.existsSync(a) || s.mkdirSync(a),
            s.writeFileSync(r, i),
            s.unlinkSync(`${r}.bak`);
        } else {
          const e = this.loadFromWebStorageBackup(t),
            i = LZString.compressToBase64(e),
            s = this.webStorageKey(t);
          localStorage.setItem(s, i), localStorage.removeItem(`${s}bak`);
        }
    }
    static saveToLocalFile(t, e) {
      const i = LZString.compressToBase64(e),
        s = require("fs"),
        a = this.localFileDirectoryPath(),
        r = this.localFilePath(t);
      s.existsSync(a) || s.mkdirSync(a), s.writeFileSync(r, i);
    ...
    static loadFromLocalFile(t) {
      let e = null;
      const i = require("fs"),
        s = this.localFilePath(t);
      return (
        i.existsSync(s) && (e = i.readFileSync(s, { encoding: "utf8" })),
        LZString.decompressFromBase64(e)
      );
    }
    static loadFromLocalBackupFile(t) {
      let e = null;
      const i = require("fs"),
        s = `${this.localFilePath(t)}.bak`;
      return (
        i.existsSync(s) && (e = i.readFileSync(s, { encoding: "utf8" })),
        LZString.decompressFromBase64(e)
      );
    ...
    static localFileBackupExists(t) {
      return require("fs").existsSync(`${this.localFilePath(t)}.bak`);
    }
    static localFileExists(t) {
      return require("fs").existsSync(this.localFilePath(t));
    }
    ...
    static saveToWebStorage(t, e) {
      const i = this.webStorageKey(t),
        s = LZString.compressToBase64(e);
      localStorage.setItem(i, s);
    }
    static loadFromWebStorage(t) {
      const e = this.webStorageKey(t),
        i = localStorage.getItem(e);
      return LZString.decompressFromBase64(i);
    }
    static loadFromWebStorageBackup(t) {
      const e = `${this.webStorageKey(t)}bak`,
        i = localStorage.getItem(e);
      return LZString.decompressFromBase64(i);
    }
    ...
    static localFileDirectoryPath() {
      const t = require("path"),
        e = t.dirname(process.mainModule.filename);
      return this.canMakeWwwSaveDirectory()
        ? t.join(e, "save/")
        : t.join(t.dirname(e), "save/");
    }
      ...
      return (
        (e =
          t < 0
            ...
            : 0 === t
            ? "global.rpgsave"
            : "file%1.rpgsave".format(t)),
        this.localFileDirectoryPath() + e
      );
    }
    static canMakeWwwSaveDirectory() {
      if (void 0 === this._canMakeWwwSaveDirectory) {
        const t = require("fs"),
          e = require("path"),
          i = e.dirname(process.mainModule.filename),
          s = e.join(i, "testDirectory/");
        try {
          t.mkdirSync(s), t.rmdirSync(s), (this._canMakeWwwSaveDirectory = !0);
        } catch (t) {
          this._canMakeWwwSaveDirectory = !1;
        }
      }
  ...
  class Game_Item {
    ...
    setObject(t) {
      DataManager.isSkill(t)
        ? (this._dataClass = "skill")
        : DataManager.isItem(t)
        ? (this._dataClass = "item")
        : DataManager.isWeapon(t)
        ? (this._dataClass = "weapon")
        : DataManager.isArmor(t)
  ...
  class Game_Action {
    ...
    repeatTargets(t) {
      const e = [],
        i = this.numRepeats();
      for (const s of t) if (s) for (let t = 0; t < i; t++) e.push(s);
      return e;
    }
    confusionTarget() {
    ...
    testItemEffect(t, { code: e, value1: i, value2: s, dataId: a }) {
      switch (e) {
        case Game_Action.EFFECT_RECOVER_HP:
          return t.hp < t.mhp || i < 0 || s < 0;
        case Game_Action.EFFECT_RECOVER_MP:
          return t.mp < t.mmp || i < 0 || s < 0;
        case Game_Action.EFFECT_ADD_STATE:
          return !t.isStateAffected(a);
        case Game_Action.EFFECT_REMOVE_STATE:
          return t.isStateAffected(a);
        case Game_Action.EFFECT_ADD_BUFF:
          return !t.isMaxBuffAffected(a);
        case Game_Action.EFFECT_ADD_DEBUFF:
          return !t.isMaxDebuffAffected(a);
        case Game_Action.EFFECT_REMOVE_BUFF:
          return t.isBuffAffected(a);
        case Game_Action.EFFECT_REMOVE_DEBUFF:
          return t.isDebuffAffected(a);
        case Game_Action.EFFECT_LEARN_SKILL:
          return t.isActor() && !t.isLearnedSkill(a);
        default:
          return !0;
      }
    }
    ...
    makeDamageValue(t, e) {
      const i = this.item(),
        s = this.evalDamageFormula(t);
      let a = this.processElementalDamage(i, s, t, e);
      return (
        this.isPhysical() && (a = this.processPhysicalDamage(i, a, t, e)),
        this.isMagical() && (a = this.processMagicalDamage(i, a, t, e)),
        s < 0 && (a = this.processRecoveryDamage(i, a, t, e)),
        e && (a = this.processCriticalDamage(i, a, t, e)),
        (a = this.processVarianceDamage(i, a, t, e)),
        (a = this.processGuardDamage(i, a, t, e)),
        (a = this.processDamageEnd(i, a, t, e)),
        a
      );
    }
    processElementalDamage(t, e, i, s) {
      return e * this.calcElementRate(i);
    }
    processPhysicalDamage(t, e, i, s) {
      return e * i.pdr;
    }
    processMagicalDamage(t, e, i, s) {
      return e * i.mdr;
    }
    processRecoveryDamage(t, e, i, s) {
      return e * i.rec;
    }
    processCriticalDamage(t, e, i, s) {
      return this.applyCritical(e);
    }
    processVarianceDamage(t, e, i, s) {
      return this.applyVariance(e, t.damage.variance);
    }
    processGuardDamage(t, e, i, s) {
      return this.applyGuard(e, i);
    }
    processDamageEnd(t, e, i, s) {
      return Math.round(e);
    }
    ...
    applyVariance(t, e) {
      const i = Math.floor(Math.max((Math.abs(t) * e) / 100, 0)),
        s = Math.randomInt(i + 1) + Math.randomInt(i + 1) - i;
      return t >= 0 ? t + s : t - s;
    }
    applyGuard(t, e) {
    ...
    itemEffectRecoverHp(t, { value1: e, value2: i }) {
      let s = (t.mhp * e + i) * t.rec;
      this.isItem() && (s *= this.subject().pha),
        (s = Math.floor(s)),
        0 !== s && (t.gainHp(s), this.makeSuccess(t));
    }
    itemEffectRecoverMp(t, { value1: e, value2: i }) {
      let s = (t.mmp * e + i) * t.rec;
      this.isItem() && (s *= this.subject().pha),
        (s = Math.floor(s)),
        0 !== s && (t.gainMp(s), this.makeSuccess(t));
    }
    ...
    itemEffectAddAttackState(t, { value1: e }) {
      this.subject()
        ...
        .forEach((i) => {
          let s = e;
          (s *= t.stateRate(i)),
            (s *= this.subject().attackStatesRate(i)),
            (s *= this.lukEffectRate(t)),
            Math.random() < s &&
    ...
    itemEffectAddNormalState(t, { value1: e, dataId: i }) {
      let s = e;
      this.isCertainHit() ||
        ((s *= t.stateRate(i)), (s *= this.lukEffectRate(t))),
        Math.random() < s &&
          (t.addState(i, this.subject()), this.makeSuccess(t));
    }
    itemEffectRemoveState(t, { value1: e, dataId: i }) {
      const s = e;
      Math.random() < s && (t.removeState(i), this.makeSuccess(t));
    }
    itemEffectAddBuff(t, { dataId: e, value1: i }) {
      t.addBuff(e, i), this.makeSuccess(t);
    }
    itemEffectAddDebuff(t, { dataId: e, value1: i }) {
      const s = t.debuffRate(e) * this.lukEffectRate(t);
      Math.random() < s && (t.addDebuff(e, i), this.makeSuccess(t));
    }
  ...
  class Scene_Base extends Stage {
    ...
    terminate() {}
    createWindowLayer() {
      const t = Graphics$1.boxWidth,
        e = Graphics$1.boxHeight,
        i = (Graphics$1.width - t) / 2,
        s = (Graphics$1.height - e) / 2;
      (this._windowLayer = new WindowLayer()),
        this._windowLayer.move(i, s, t, e),
  ...
  class Sprite_Timer extends Sprite {
    ...
    updatePosition() {
      (this.x = Graphics$1.width - this.bitmap.width), (this.y = 0);
    }
    updateVisibility() {
  ...
  class Spriteset_Base extends Sprite {
    ...
    createWebGLToneChanger() {
      const t = Graphics$1.width + 96,
        e = Graphics$1.height + 96;
    ...
    createPictures() {
      ...
      (this._pictureContainer = new Sprite()),
        this._pictureContainer.setFrame(i, s, t, e);
  ...
  class Sprite_Animation extends Sprite {
    ...
    loadBitmaps() {
      const t = this._animation.animation1Name,
        ...
        s = this._animation.animation2Hue;
      (this._bitmap1 = ImageManager.loadAnimation(t, i)),
        (this._bitmap2 = ImageManager.loadAnimation(e, s));
    }
    isReady() {
      return (
        this._bitmap1 &&
    ...
    updateCellSprite(t, e) {
      ...
      if (i >= 0) {
        const s = (i % 5) * 192,
          a = 192 * Math.floor((i % 100) / 5),
          r = this._mirror;
        (t.bitmap = i < 100 ? this._bitmap1 : this._bitmap2),
          t.setFrame(s, a, 192, 192),
          (t.x = e[1]),
          (t.y = e[2]),
          (t.rotation = (e[4] * Math.PI) / 180),
          (t.scale.x = e[3] / 100),
          e[5] && (t.scale.x *= -1),
          r && ((t.x *= -1), (t.rotation *= -1), (t.scale.x *= -1)),
          (t.scale.y = e[3] / 100),
          (t.opacity = e[6]),
          (t.blendMode = e[7]),
          (t.visible = !0);
    ...
    processTimingData({
      ...
      const a = t * this._rate;
      switch (e) {
        case 1:
          this.startFlash(i, a);
          break;
        case 2:
          this.startScreenFlash(i, a);
          break;
        case 3:
          this.startHiding(a);
      }
      !this._duplicated && s && AudioManager.playSe(s);
    }
    startFlash(t, e) {
  ...
  class Sprite_Base extends Sprite {
    ...
    startAnimation(t, e, i) {
      ...
      s.setup(this._effectTarget, t, e, i),
        this.parent.addChild(s),
        this._animationSprites.push(s);
    }
    isAnimationPlaying() {
    ...
    createDigits(t, e) {
      ...
      for (let t = 0; t < i.length; t++) {
        const e = this.createChildSprite(),
          n = Number(i[t]);
        e.setFrame(n * a, s * r, a, r),
          (e.x = (t - (i.length - 1) / 2) * a),
          (e.dy = -t);
      }
    }
    createChildSprite() {
    ...
    setupAnimation() {
      for (; this._battler.isAnimationRequested(); ) {
        const t = this._battler.shiftAnimation(),
          ...
          i = t.mirror,
          s = 3 === e.position ? 0 : t.delay;
        this.startAnimation(e, i, s);
  ...
  class Sprite_StateIcon extends Sprite {
    ...
    updateFrame() {
      const t = Sprite_StateIcon._iconWidth,
        ...
        s = Math.floor(this._iconIndex / 16) * e;
      this.setFrame(i, s, t, e);
    }
  }
    ...
    updateFrame() {
      if (this._weaponImageId > 0) {
        const t = (this._weaponImageId - 1) % 12,
          e = 96,
          i = 64,
          s = (3 * Math.floor(t / 6) + this._pattern) * e,
          a = Math.floor(t % 6) * i;
        this.setFrame(s, a, e, i);
    ...
    updateFrame() {
      ...
      if (t) {
        const e = this._motion ? this._motion.index : 0,
          ...
          s = t.width / 9,
          a = t.height / 6,
          r = 3 * Math.floor(e / 6) + i,
          n = e % 6;
        this._mainSprite.setFrame(r * s, n * a, s, a);
      }
    }
    updateMove() {
      const t = this._mainSprite.bitmap;
  ...
  class Spriteset_Battle extends Spriteset_Base {
    ...
    createBattleField() {
      ...
      (this._battleField = new Sprite()),
        this._battleField.setFrame(i, s, t, e),
        (this._battleField.x = i),
        (this._battleField.y = s),
        this._baseSprite.addChild(this._battleField);
    }
    createBattleback() {
      const t = -this._battleField.x - 32,
        e = -this._battleField.y - 32,
        i = Graphics$1.width + 64,
        s = Graphics$1.height + 64;
      (this._back1Sprite = new TilingSprite()),
        (this._back2Sprite = new TilingSprite()),
        (this._back1Sprite.bitmap = this.battleback1Bitmap()),
        (this._back2Sprite.bitmap = this.battleback2Bitmap()),
        this._back1Sprite.move(t, e, i, s),
        this._back2Sprite.move(t, e, i, s),
    ...
    locateBattleback() {
      ...
      (i.origin.x = i.x + (i.bitmap.width - t) / 2),
        (s.origin.x = i.y + (s.bitmap.width - t) / 2),
        self.$gameSystem.isSideView() &&
          ((i.origin.y = i.x + i.bitmap.height - e),
          (s.origin.y = i.y + s.bitmap.height - e));
    }
    battleback1Bitmap() {
  ...
  class Window_Base extends Window {
    ...
    drawText(t, e, i, s, a) {
      this.contents.drawText(t, e, i, s, this.lineHeight(), a);
    }
    textWidth(t) {
    ...
    drawTextEx(t, e, i) {
      if (t) {
        const s = { index: 0, x: e, y: i, left: e };
        for (
          s.text = this.convertEscapeCharacters(t),
            s.height = this.calcTextHeight(s, !1),
            this.resetFontSettings();
    ...
    calcTextHeight({ text: t, index: e }, i) {
      const s = this.contents.fontSize;
      let a = 0;
      const r = t.slice(e).split("\n"),
        n = i ? r.length : 1;
      for (let t = 0; t < n; t++) {
        ...
        for (;;) {
          const s = i.exec(r[t]);
          if (!s) break;
          "{" === s[0] && this.makeFontBigger(),
            "}" === s[0] && this.makeFontSmaller(),
            e < this.contents.fontSize && (e = this.contents.fontSize);
        }
        a += e + 8;
      }
      return (this.contents.fontSize = s), a;
    }
    drawIcon(t, e, i) {
      const s = ImageManager.loadSystem("IconSet"),
        a = Window_Base._iconWidth,
        r = Window_Base._iconHeight,
        n = (t % 16) * a,
        h = Math.floor(t / 16) * r;
      this.contents.blt(s, n, h, a, r, e, i);
    }
    drawFace(
      t,
      e,
      i,
      s,
      a = Window_Base._faceWidth,
      r = Window_Base._faceHeight
    ) {
      const n = ImageManager.loadFace(t),
        h = Window_Base._faceWidth,
        o = Window_Base._faceHeight,
        l = Math.min(a, h),
        c = Math.min(r, o),
        d = Math.floor(i + Math.max(a - h, 0) / 2),
        m = Math.floor(s + Math.max(r - o, 0) / 2),
        u = (e % 4) * h + (h - l) / 2,
        p = Math.floor(e / 4) * o + (o - c) / 2;
      this.contents.blt(n, u, p, l, c, d, m);
    }
    drawCharacter(t, e, i, s) {
      const a = ImageManager.loadCharacter(t),
        r = ImageManager.isBigCharacter(t),
        n = a.width / (r ? 3 : 12),
        h = a.height / (r ? 4 : 8),
        o = r ? 0 : e,
        l = ((o % 4) * 3 + 1) * n,
        c = 4 * Math.floor(o / 4) * h;
      this.contents.blt(a, l, c, n, h, i - n / 2, s - h);
    }
    drawGauge(t, e, i, s, a, r) {
      const n = Math.floor(i * s),
        h = e + this.lineHeight() - 8;
      this.contents.fillRect(t, h, i, 6, this.gaugeBackColor()),
        this.contents.gradientFillRect(t, h, n, 6, a, r);
    }
    hpColor(t) {
      return t.isDead()
        ? this.deathColor()
    ...
    drawActorFace(t, e, i, s, a) {
      this.drawFace(t.faceName(), t.faceIndex(), e, i, s, a);
    }
    drawActorName(t, e, i, s = 168) {
      this.changeTextColor(this.hpColor(t)), this.drawText(t.name(), e, i, s);
    }
    drawActorClass(t, e, i, s = 168) {
      this.resetTextColor(), this.drawText(t.currentClass().name, e, i, s);
    }
    drawActorNickname(t, e, i, s = 270) {
      this.resetTextColor(), this.drawText(t.nickname(), e, i, s);
    }
    ...
    drawActorIcons(t, e, i, s = 144) {
      const a = t.allIcons().slice(0, Math.floor(s / Window_Base._iconWidth));
      for (let t = 0; t < a.length; t++)
        this.drawIcon(a[t], e + Window_Base._iconWidth * t, i + 2);
    }
    drawCurrentAndMax(t, e, i, s, a, r, n) {
      const h = this.textWidth("HP"),
        o = this.textWidth("0000"),
        l = this.textWidth("/"),
        c = i + a - o,
        d = c - l,
        m = d - o;
      m >= i + h
        ? (this.changeTextColor(r),
          this.drawText(t, m, s, o, "right"),
          this.changeTextColor(n),
          this.drawText("/", d, s, l, "right"),
          this.drawText(e, c, s, o, "right"))
        : (this.changeTextColor(r), this.drawText(t, c, s, o, "right"));
    }
    drawActorHp(t, e, i, s = 186) {
      const a = this.hpGaugeColor1(),
        r = this.hpGaugeColor2();
      this.drawGauge(e, i, s, t.hpRate(), a, r),
        this.changeTextColor(this.systemColor()),
    ...
    drawActorMp(t, e, i, s = 186) {
      const a = this.mpGaugeColor1(),
        r = this.mpGaugeColor2();
      this.drawGauge(e, i, s, t.mpRate(), a, r),
        this.changeTextColor(this.systemColor()),
    ...
    drawActorTp(t, e, i, s = 96) {
      const a = this.tpGaugeColor1(),
        r = this.tpGaugeColor2();
      this.drawGauge(e, i, s, t.tpRate(), a, r),
        this.changeTextColor(this.systemColor()),
        this.drawText(TextManager.tpA, e, i, 44),
        this.changeTextColor(this.tpColor(t)),
        this.drawText(t.tp, e + s - 64, i, 64, "right");
    }
    drawActorSimpleStatus(t, e, i, s) {
      const a = this.lineHeight(),
        r = e + 180,
        n = Math.min(200, s - 180 - this.textPadding());
      this.drawActorName(t, e, i),
        this.drawActorLevel(t, e, i + 1 * a),
        this.drawActorIcons(t, e, i + 2 * a),
        this.drawActorClass(t, r, i),
        this.drawActorHp(t, r, i + 1 * a, n),
        this.drawActorMp(t, r, i + 2 * a, n);
    }
    drawItemName(t, e, i, s = 312) {
      if (t) {
        const a = Window_Base._iconWidth + 4;
        this.resetTextColor(),
          this.drawIcon(t.iconIndex, e + 2, i + 2),
          this.drawText(t.name, e + a, i, s - a);
      }
    }
    drawCurrencyValue(t, e, i, s, a) {
      const r = Math.min(80, this.textWidth(e));
      this.resetTextColor(),
        this.drawText(t, i, s, a - r - 6, "right"),
        this.changeTextColor(this.systemColor()),
        this.drawText(e, i + a - r, s, r, "right");
    }
    ...
    refreshDimmerBitmap() {
      if (this._dimmerSprite) {
        const t = this._dimmerSprite.bitmap,
          ...
          r = this.dimColor2();
        t.resize(e, i),
          t.gradientFillRect(0, 0, e, s, r, a, !0),
          t.fillRect(0, s, e, i - 2 * s, a),
          t.gradientFillRect(0, i - s, e, s, a, r, !0),
  ...
  class Window_Gold extends Window_Base {
    ...
    initialize(t, e) {
      const i = this.windowWidth(),
        s = this.windowHeight();
      super.initialize(t, e, i, s), this.refresh();
    }
    windowWidth() {
      return 240;
    }
  ...
  class Window_Selectable extends Window_Base {
    ...
    cursorDown(t) {
      const e = this.index(),
        i = this.maxItems(),
        s = this.maxCols();
      (e < i - s || (t && 1 === s)) && this.select((e + s) % i);
    }
    cursorUp(t) {
      const e = this.index(),
        i = this.maxItems(),
        s = this.maxCols();
      (e >= s || (t && 1 === s)) && this.select((e - s + i) % i);
    }
    cursorRight(t) {
      const e = this.index(),
        i = this.maxItems();
    ...
    onTouch(t) {
      const e = this.index(),
        ...
        s = this.canvasToLocalY(TouchInput.y),
        a = this.hitTest(i, s);
      a >= 0
        ? a === this.index()
          ? t && this.isTouchOkEnabled() && this.processOk()
          : this.isCursorMovable() && this.select(a)
        : this._stayCount >= 10 &&
          (s < this.padding
            ? this.cursorUp()
            : s >= this.height - this.padding && this.cursorDown()),
        ...
        for (let t = 0; t < this.maxPageItems(); t++) {
          ...
          if (e < this.maxItems()) {
            const t = this.itemRect(e),
              a = t.x + t.width,
              r = t.y + t.height;
            if (i >= t.x && s >= t.y && i < a && s < r) return e;
          }
        }
      }
      return -1;
    ...
    isContentsArea(t, e) {
      const i = this.padding,
        s = this.padding,
        a = this.width - this.padding,
        r = this.height - this.padding;
      return t >= i && e >= s && t < a && e < r;
    }
    isTouchOkEnabled() {
  ...
  class Window_Command extends Window_Selectable {
    ...
    addCommand(t, e, i, s) {
      void 0 === i && (i = !0),
        void 0 === s && (s = null),
        this._list.push({ name: t, symbol: e, enabled: i, ext: s });
    }
    commandName(t) {
  ...
  class Window_ChoiceList extends Window_Command {
    ...
    windowWidth() {
      ...
      return Math.min(t, Graphics$1.boxWidth);
    }
    numVisibleRows() {
      const t = this._messageWindow.y,
        e = this._messageWindow.height,
        i = Graphics$1.boxHeight / 2;
      let s = self.$gameMessage.choices().length,
        a = 8;
      return t < i && t + e > i && (a = 4), s > a && (s = a), s;
    }
    maxChoiceWidth() {
      let t = 96;
      const e = self.$gameMessage.choices();
      for (let i = 0; i < e.length; i++) {
        const s = this.textWidthEx(e[i]) + 2 * this.textPadding();
        t < s && (t = s);
      }
      return t;
    }
  ...
  class Sprite_Button extends Sprite {
    ...
    setColdFrame(t, e, i, s) {
      this._coldFrame = new Rectangle(t, e, i, s);
    }
    setHotFrame(t, e, i, s) {
      this._hotFrame = new Rectangle(t, e, i, s);
    }
    setClickHandler(t) {
  ...
  class Window_NumberInput extends Window_Selectable {
    ...
    createButtons() {
      ...
      for (let e = 0; e < 3; e++) {
        const i = new Sprite_Button(),
          ...
          a = 48 * (2 === e ? 2 : 1);
        (i.bitmap = t),
          i.setColdFrame(s, 0, a, 48),
          i.setHotFrame(s, 48, a, 48),
          (i.visible = !1),
    ...
    buttonY() {
      return this._messageWindow.y >= Graphics$1.boxHeight / 2
    ...
    changeDigit(t) {
      ...
      let s = Math.floor(this._number / i) % 10;
      (this._number -= s * i),
        (s = t ? (s + 1) % 10 : (s + 9) % 10),
        (this._number += s * i),
        this.refresh(),
  ...
  class Window_ItemList extends Window_Selectable {
    ...
    includes(t) {
      switch (this._category) {
        case "item":
          return DataManager.isItem(t) && 1 === t.itypeId;
        case "weapon":
          return DataManager.isWeapon(t);
        case "armor":
          return DataManager.isArmor(t);
        case "keyItem":
          return DataManager.isItem(t) && 2 === t.itypeId;
        default:
          return !1;
      }
    }
    ...
    drawItem(t) {
      ...
      if (e) {
        const i = this.numberWidth(),
          s = this.itemRect(t);
        (s.width -= this.textPadding()),
          this.changePaintOpacity(this.isEnabled(e)),
          this.drawItemName(e, s.x, s.y, s.width - i),
          this.drawItemNumber(e, s.x, s.y, s.width),
    ...
    drawItemNumber(t, e, i, s) {
      this.needsNumber() &&
        (this.drawText(":", e, i, s - this.textWidth("00"), "right"),
        this.drawText(self.$gameParty.numItems(t), e, i, s, "right"));
    }
    updateHelp() {
  ...
  class Window_EventItem extends Window_ItemList {
    ...
    updatePlacement() {
      this._messageWindow.y >= Graphics$1.boxHeight / 2
        ? (this.y = 0)
  ...
  class Window_Message extends Window_Base {
    ...
    updatePlacement() {
      (this._positionType = self.$gameMessage.positionType()),
        (this.y =
          (this._positionType * (Graphics$1.boxHeight - this.height)) / 2),
        (this._goldWindow.y =
  ...
  class Window_BattleLog extends Window_Selectable {
    ...
    showNormalAnimation(t, e, i) {
      if (self.$dataAnimations[e]) {
        let s = this.animationBaseDelay();
        const a = this.animationNextDelay();
        t.forEach((t) => {
          t.startAnimation(e, i, s), (s += a);
        });
      }
    }
    animationBaseDelay() {
    ...
    startAction(t, e, i) {
      ...
      this.push("performActionStart", t, e),
        ...
        this.push("showAnimation", t, i.clone(), s.animationId),
        this.displayAction(t, s);
    }
    endAction(t) {
    ...
    displayAddedStates(t) {
      t.result()
        .addedStateObjects()
        .forEach(function ({ message1: e, message2: i, id: s }) {
          const a = t.isActor() ? e : i;
          s === t.deathStateId() && this.push("performCollapse", t),
            a &&
              (this.push("popBaseLine"),
              this.push("pushBaseLine"),
              this.push("addText", t.name() + a),
    ...
    makeHpDamageText(t) {
      ...
      return i > 0 && e.drain
        ? ((a = s ? TextManager.actorDrain : TextManager.enemyDrain),
          a.format(t.name(), TextManager.hp, i))
        : i > 0
        ? ((a = s ? TextManager.actorDamage : TextManager.enemyDamage),
          a.format(t.name(), i))
        : i < 0
        ? ((a = s ? TextManager.actorRecovery : TextManager.enemyRecovery),
          a.format(t.name(), TextManager.hp, -i))
        : ((a = s ? TextManager.actorNoDamage : TextManager.enemyNoDamage),
          a.format(t.name()));
    }
    makeMpDamageText(t) {
      const e = t.result(),
      ...
      return i > 0 && e.drain
        ? ((a = s ? TextManager.actorDrain : TextManager.enemyDrain),
          a.format(t.name(), TextManager.mp, i))
        : i > 0
        ? ((a = s ? TextManager.actorLoss : TextManager.enemyLoss),
          a.format(t.name(), TextManager.mp, i))
        : i < 0
        ? ((a = s ? TextManager.actorRecovery : TextManager.enemyRecovery),
          a.format(t.name(), TextManager.mp, -i))
    ...
    makeTpDamageText(t) {
      ...
      let s;
      return e > 0
        ? ((s = i ? TextManager.actorLoss : TextManager.enemyLoss),
          s.format(t.name(), TextManager.tp, e))
        : e < 0
        ? ((s = i ? TextManager.actorGain : TextManager.enemyGain),
          s.format(t.name(), TextManager.tp, -e))
        : "";
    }
  }
  class Window_PartyCommand extends Window_Command {
    ...
    initialize() {
      const t = Graphics$1.boxHeight - this.windowHeight();
  ...
  class Window_BattleStatus extends Window_Selectable {
    ...
    initialize() {
      const t = this.windowWidth(),
        e = this.windowHeight(),
        i = Graphics$1.boxWidth - t,
        s = Graphics$1.boxHeight - e;
      super.initialize(i, s, t, e), this.refresh(), (this.openness = 0);
    }
    windowWidth() {
    ...
    drawBasicArea({ x: t, y: e, width: i }, s) {
      this.drawActorName(s, t + 0, e, 150),
        this.drawActorIcons(s, t + 156, e, i - 156);
    }
    drawGaugeArea(t, e) {
  ...
  class Window_BattleEnemy extends Window_Selectable {
    ...
    initialize(t, e) {
      ...
      const i = this.windowWidth(),
        s = this.windowHeight();
      super.initialize(t, e, i, s), this.refresh(), this.hide();
    }
    windowWidth() {
      return Graphics$1.boxWidth - 192;
    }
  ...
  class Window_SkillList extends Window_Selectable {
    ...
    drawItem(t) {
      ...
      if (e) {
        ...
        (s.width -= this.textPadding()),
          this.changePaintOpacity(this.isEnabled(e)),
          this.drawItemName(e, s.x, s.y, s.width - i),
          this.drawSkillCost(e, s.x, s.y, s.width),
          this.changePaintOpacity(1);
      }
    }
    costWidth() {
    ...
    drawSkillCost(t, e, i, s) {
      this._actor.skillTpCost(t) > 0
        ? (this.changeTextColor(this.tpCostColor()),
          this.drawText(this._actor.skillTpCost(t), e, i, s, "right"))
        : this._actor.skillMpCost(t) > 0 &&
          (this.changeTextColor(this.mpCostColor()),
          this.drawText(this._actor.skillMpCost(t), e, i, s, "right"));
    }
    updateHelp() {
      this.setHelpWindowItem(this.item());
    }
  ...
  class Window_BattleSkill extends Window_SkillList {
    ...
    initialize(t, e, i, s) {
      super.initialize(t, e, i, s), this.hide();
    }
    show() {
  ...
  class Scene_Battle extends Scene_Base {
    ...
    createItemWindow() {
      const t = this._helpWindow.y + this._helpWindow.height,
        e = this._statusWindow.y - t;
      (this._itemWindow = new Window_BattleItem(0, t, Graphics$1.boxWidth, e)),
  ...
  class Window_MenuStatus extends Window_Selectable {
    ...
    drawItemStatus(t) {
      const e = self.$gameParty.members()[t],
        i = this.itemRect(t),
        s = i.x + 162,
        a = i.y + i.height / 2 - 1.5 * this.lineHeight(),
        r = i.width - s - this.textPadding();
      this.drawActorSimpleStatus(e, s, a, r);
    }
    processOk() {
      super.processOk(),
  ...
  class Scene_ItemBase extends Scene_MenuBase {
    ...
    showSubWindow(t) {
      (t.x = this.isCursorLeft() ? Graphics$1.boxWidth - t.width : 0),
        t.show(),
        t.activate();
    }
    hideSubWindow(t) {
  ...
  class Scene_Item extends Scene_ItemBase {
    ...
    createItemWindow() {
      const t = this._categoryWindow.y + this._categoryWindow.height,
        e = Graphics$1.boxHeight - t;
      (this._itemWindow = new Window_ItemList(0, t, Graphics$1.boxWidth, e)),
        this._itemWindow.setHelpWindow(this._helpWindow),
        this._itemWindow.setHandler("ok", this.onItemOk.bind(this)),
        this._itemWindow.setHandler("cancel", this.onItemCancel.bind(this)),
        this.addWindow(this._itemWindow),
  ...
  class Window_SkillStatus extends Window_Base {
    ...
    initialize(t, e, i, s) {
      super.initialize(t, e, i, s), (this._actor = null);
    }
    setActor(t) {
    ...
    refresh() {
      if ((this.contents.clear(), this._actor)) {
        const t = this.width - 2 * this.padding,
          ...
          s = t - 162 - this.textPadding();
        this.drawActorFace(this._actor, 0, 0, 144, e),
          this.drawActorSimpleStatus(this._actor, 162, i, s);
      }
    }
  }
  class Scene_Skill extends Scene_ItemBase {
    ...
    createStatusWindow() {
      const t = this._skillTypeWindow.width,
        ...
        i = Graphics$1.boxWidth - t,
        s = this._skillTypeWindow.height;
      (this._statusWindow = new Window_SkillStatus(t, e, i, s)),
        this._statusWindow.reserveFaceImages(),
        this.addWindow(this._statusWindow);
    }
    createItemWindow() {
      const t = this._statusWindow.y + this._statusWindow.height,
        e = Graphics$1.boxWidth,
        i = Graphics$1.boxHeight - t;
  ...
  class Window_EquipStatus extends Window_Base {
    ...
    drawNewParam(t, e, i) {
      const s = this._tempActor.param(i),
        a = s - this._actor.param(i);
      this.changeTextColor(this.paramchangeTextColor(a)),
        this.drawText(s, t, e, 48, "right");
    }
  }
  ...
  class Window_EquipSlot extends Window_Selectable {
    ...
    initialize(t, e, i, s) {
      super.initialize(t, e, i, s), (this._actor = null), this.refresh();
    }
    setActor(t) {
      this._actor !== t && ((this._actor = t), this.refresh());
    }
  ...
  class Window_EquipItem extends Window_ItemList {
    ...
    initialize(t, e, i, s) {
      super.initialize(t, e, i, s), (this._actor = null), (this._slotId = 0);
    }
    setActor(t) {
      this._actor !== t &&
        ((this._actor = t), this.refresh(), this.resetScroll());
  ...
  class Scene_Equip extends Scene_MenuBase {
    ...
    createCommandWindow() {
      const t = this._statusWindow.width,
        e = this._helpWindow.height,
        i = Graphics$1.boxWidth - this._statusWindow.width;
    ...
    createSlotWindow() {
      const t = this._statusWindow.width,
        ...
        i = Graphics$1.boxWidth - this._statusWindow.width,
        s = this._statusWindow.height - this._commandWindow.height;
      (this._slotWindow = new Window_EquipSlot(t, e, i, s)),
    ...
    drawParameters(t, e) {
      ...
      for (let s = 0; s < 6; s++) {
        const a = s + 2,
          r = e + i * s;
        this.changeTextColor(this.systemColor()),
          this.drawText(TextManager.param(a), t, r, 160),
          this.resetTextColor(),
          this.drawText(this._actor.param(a), t + 160, r, 60, "right");
      }
    }
    drawExpInfo(t, e) {
      const i = this.lineHeight(),
        s = TextManager.expTotal.format(TextManager.exp),
        a = TextManager.expNext.format(TextManager.level);
      let r = this._actor.currentExp(),
        n = this._actor.nextRequiredExp();
      this._actor.isMaxLevel() && ((r = "-------"), (n = "-------")),
        this.changeTextColor(this.systemColor()),
        this.drawText(s, t, e + 0 * i, 270),
        this.drawText(a, t, e + 2 * i, 270),
        this.resetTextColor(),
        this.drawText(r, t, e + 1 * i, 270, "right"),
        this.drawText(n, t, e + 3 * i, 270, "right");
    }
    drawEquipments(t, e) {
      const i = this._actor.equips(),
        s = Math.min(i.length, this.maxEquipmentLines());
      for (let a = 0; a < s; a++)
        this.drawItemName(i[a], t, e + this.lineHeight() * a);
    }
    drawProfile(t, e) {
  ...
  class Window_Options extends Window_Command {
    ...
    drawItem(t) {
      ...
      this.resetTextColor(),
        this.changePaintOpacity(this.isCommandEnabled(t)),
        this.drawText(this.commandName(t), e.x, e.y, s, "left"),
        this.drawText(this.statusText(t), e.x + s, e.y, i, "right");
    }
    statusWidth() {
      return 120;
    }
    ...
    cursorRight(t) {
      ...
      let s = this.getConfigValue(i);
      this.isVolumeSymbol(i)
        ? ((s += this.volumeOffset()),
          (s = s.clamp(0, 100)),
          this.changeValue(i, s))
  ...
  class Window_SavefileList extends Window_Selectable {
    ...
    initialize(t, e, i, s) {
      super.initialize(t, e, i, s), this.activate(), (this._mode = null);
    }
    setMode(t) {
      this._mode = t;
    }
    ...
    drawItem(t) {
      const e = t + 1,
        i = DataManager.isThisGameFile(e),
        s = DataManager.loadSavefileInfo(e),
        a = this.itemRectForText(t);
      this.resetTextColor(),
        "load" === this._mode && this.changePaintOpacity(i),
        this.drawFileId(e, a.x, a.y),
        s &&
          (this.changePaintOpacity(i),
          this.drawContents(s, a, i),
          this.changePaintOpacity(!0));
    }
    drawFileId(t, e, i) {
      DataManager.isAutoSaveFileId(t)
    ...
    drawContents(t, { y: e, height: i, width: s, x: a }, r) {
      const n = e + i;
      s >= 420 &&
        (this.drawGameTitle(t, a + 192, e, s - 192),
        r && this.drawPartyCharacters(t, a + 220, n - 4));
      const h = this.lineHeight(),
        o = n - h;
      o >= h && this.drawPlaytime(t, a, o, s);
    }
    drawGameTitle({ title: t }, e, i, s) {
      t && this.drawText(t, e, i, s);
    }
    drawPartyCharacters({ characters: t }, e, i) {
      t &&
        t.forEach((t, s) => {
          this.drawCharacter(t[0], t[1], e + 48 * s, i);
        });
    }
    drawPlaytime({ playtime: t }, e, i, s) {
      t && this.drawText(t, e, i, s, "right");
    }
    playOkSound() {}
  }
  ...
  class Scene_Save extends Scene_File {
    ...
    onSavefileOk() {
      DataManager.isAutoSaveFileId(this.savefileId())
        ? this.onSaveFailure()
        : (super.onSavefileOk(),
  ...
  class Window_DebugRange extends Window_Selectable {
    ...
    drawItem(t) {
      ...
      t < this._maxSwitches
        ? ((i = 10 * t + 1), (s = "S"))
        : ((i = 10 * (t - this._maxSwitches) + 1), (s = "V"));
      const a = i + 9;
      (s += ` [${i.padZero(4)}-${a.padZero(4)}]`),
        this.drawText(s, e.x, e.y, e.width);
    }
    isCancelTriggered() {
      return (
  ...
  class Window_DebugEdit extends Window_Selectable {
    ...
    drawItem(t) {
      const e = this._topId + t,
        i = `${e.padZero(4)}:`,
        s = this.textWidth(i),
        a = this.textWidth("-00000000"),
        r = this.itemName(e),
        n = this.itemStatus(e),
        h = this.itemRectForText(t);
      this.resetTextColor(),
        this.drawText(i, h.x, h.y, h.width),
        (h.x += s),
        (h.width -= s + a),
        this.drawText(r, h.x, h.y, h.width),
        this.drawText(n, h.x + h.width, h.y, a, "right");
    }
    itemName(t) {
  ...
  class Scene_Debug extends Scene_MenuBase {
    ...
    createDebugHelpWindow() {
      const t = this._editWindow.x,
        ...
        s = Graphics$1.boxHeight - e;
      (this._debugHelpWindow = new Window_Base(t, e, i, s)),
  ...
  class Window_MapName extends Window_Base {
    ...
    drawBackground(t, e, i, s) {
      const a = this.dimColor1(),
        r = this.dimColor2();
      this.contents.gradientFillRect(t, e, i / 2, s, r, a),
        this.contents.gradientFillRect(t + i / 2, e, i / 2, s, a, r);
    }
  }
  ...
  class Sprite_Character extends Sprite_Base {
    ...
    updateTileFrame() {
      const t = this.patternWidth(),
        ...
        s = (Math.floor((this._tileId % 256) / 8) % 16) * e;
      this.setFrame(i, s, t, e);
    }
    updateCharacterFrame() {
      const t = this.patternWidth(),
        ...
        s = (this.characterBlockY() + this.characterPatternY()) * e;
      if ((this.updateHalfBodySprites(), this._bushDepth > 0)) {
        const a = this._bushDepth;
        this._upperBody.setFrame(i, s, t, e - a),
          this._lowerBody.setFrame(i, s + e - a, t, a),
          this.setFrame(i, s, 0, e);
      } else this.setFrame(i, s, t, e);
    }
    characterBlockX() {
  ...
  class Sprite_Destination extends Sprite {
    ...
    updatePosition() {
      const t = self.$gameMap.tileWidth(),
        ...
        s = self.$gameTemp.destinationY();
      (this.x = (self.$gameMap.adjustX(i) + 0.5) * t),
        (this.y = (self.$gameMap.adjustY(s) + 0.5) * e);
    }
    updateAnimation() {
      this._frameCount++,
        (this._frameCount %= 20),
  ...
  class Spriteset_Map extends Spriteset_Base {
    ...
    updateParallax() {
      this._parallaxName !== self.$gameMap.parallaxName() &&
        ((this._parallaxName = self.$gameMap.parallaxName()),
        this._parallax.bitmap && 1 != Graphics$1.isWebGL()
    ...
    updateEncounterEffect() {
      if (this._encounterEffectDuration > 0) {
        ...
        const t = this.encounterEffectSpeed(),
          ...
          i = e / t,
          s = (20 * (i - 1) * i + 5) * i + 1,
          a = self.$gamePlayer.screenX(),
          r = self.$gamePlayer.screenY() - 24;
        2 === e &&
          (self.$gameScreen.setZoom(a, r, 1),
          this.snapForBattleBackground(),
          this.startFlashForEncounter(t / 2)),
          self.$gameScreen.setZoom(a, r, s),
  ...
  class Scene_Title extends Scene_Base {
    ...
    drawGameTitle() {
      const t = Graphics$1.height / 4,
        e = Graphics$1.width - 40,
    ...
    centerSprite(t) {
      (t.x = Graphics$1.width / 2),
        (t.y = Graphics$1.height / 2),
        (t.anchor.x = 0.5),
        (t.anchor.y = 0.5);
    }
    createCommandWindow() {
  ...
  class BattleManager {
    ...
    static changeActor(t, e) {
      ...
      this._actorIndex = t;
      const s = this.actor();
      i && i.setActionState(e), s && s.setActionState("inputting");
    }
    static startBattle() {
      (this._phase = "start"),
  ...
  class Scene_Boot extends Scene_Base {
    ...
    isGameFontLoaded() {
      if (Graphics$1.isFontLoaded("GameFont")) return !0;
      if (
        !Graphics$1.canUseCssFontLoading() &&
  ...
  class Game_Picture {
    ...
    move(t, e, i, s, a, r, n, h) {
      (this._origin = t),
        (this._targetX = e),
        (this._targetY = i),
        (this._targetScaleX = s),
        (this._targetScaleY = a),
        (this._targetOpacity = r),
        (this._blendMode = n),
        (this._duration = h);
    }
    rotate(t) {
  ...
  class Game_Screen {
    ...
    showPicture(t, e, i, s, a, r, n, h, o) {
      const l = this.realPictureId(t),
        c = new Game_Picture();
      c.show(e, i, s, a, r, n, h, o), (this._pictures[l] = c);
    }
    movePicture(t, e, i, s, a, r, n, h, o) {
      const l = this.picture(t);
      l && l.move(e, i, s, a, r, n, h, o);
    }
    rotatePicture(t, e) {
    ...
    canUse(t) {
      return (
        ...
        (DataManager.isSkill(t)
          ...
          : !!DataManager.isItem(t) && this.meetsItemConditions(t))
      );
    }
    canEquip(t) {
      return (
        ...
        (DataManager.isWeapon(t)
          ...
          : !!DataManager.isArmor(t) && this.canEquipArmor(t))
      );
    }
  ...
  class Game_Battler extends Game_BattlerBase {
    ...
    startAnimation(t, e, i) {
      const s = { animationId: t, mirror: e, delay: i };
    ...
    useItem(t) {
      DataManager.isSkill(t)
        ? this.paySkillCost(t)
        : DataManager.isItem(t) && this.consumeItem(t);
    }
    consumeItem(t) {
  ...
  class Game_Actor extends Game_Battler {
    ...
    expForLevel(t) {
      const e = this.currentClass(),
        ...
        r = e.expParams[3];
      return Math.round(
        (i * Math.pow(t - 1, 0.9 + a / 250) * t * (t + 1)) /
          (6 + Math.pow(t, 2) / 50 / r) +
          (t - 1) * s
      );
    }
    ...
    initEquips(t) {
      ...
      for (let s = 0; s < t.length; s++)
        s < i && this._equips[s].setEquip(1 === e[s], t[s]);
    ...
    weapons() {
      return this.equips().filter((t) => t && DataManager.isWeapon(t));
    }
    armors() {
      return this.equips().filter((t) => t && DataManager.isArmor(t));
    }
    hasWeapon(t) {
    ...
    releaseUnequippableItems(t) {
      for (;;) {
        ...
        if (
          (i.forEach((i, a) => {
            !i ||
              (this.canEquip(i) && i.etypeId === e[a]) ||
              (t || this.tradeItemWithParty(null, i),
              this._equips[a].setObject(null),
              (s = !0));
          }),
          !s)
        )
    ...
    paramPlus(t) {
      ...
      const i = this.equips();
      for (const s of i) s && (e += s.params[t]);
      return e;
    }
    attackAnimationId1() {
    ...
    changeExp(t, e) {
      ...
      e && this._level > i && this.displayLevelUp(this.findNewSkills(s)),
        this.refresh();
    }
    levelUp() {
      this._level++,
    ...
    makeAutoBattleActions() {
      for (let t = 0; t < this.numActions(); t++) {
        ...
        for (let s = 0; s < e.length; s++) {
          const a = e[s].evaluate();
          a > i && ((i = a), this.setAction(t, e[s]));
        }
      }
  ...
  class Game_Party extends Game_Unit {
    ...
    setupBattleTestMembers() {
      self.$dataSystem.testBattlers.forEach(function ({
        ...
        const s = self.$gameActors.actor(t);
        s &&
          (s.changeLevel(e, !1),
          s.initEquips(i),
          s.recoverAll(),
    ...
    gainItem(t, e, i) {
      const s = this.itemContainer(t);
      if (s) {
        const a = this.numItems(t) + e;
        (s[t.id] = a.clamp(0, this.maxItems(t))),
          0 === s[t.id] && delete s[t.id],
          i && a < 0 && this.discardMembersEquip(t, -a),
          self.$gameMap.requestRefresh();
      }
    }
  ...
  class Game_Enemy extends Game_Battler {
    ...
    makeDropItems() {
      return this.enemy().dropItems.reduce(
        (t, { kind: e, denominator: i, dataId: s }) =>
          e > 0 && Math.random() * i < this.dropItemRate()
            ? t.concat(this.itemObject(e, s))
            : t,
        []
      );
    }
    ...
    meetsCondition({
      ...
      switch (i) {
        case 1:
          return this.meetsTurnCondition(s, a);
        case 2:
          return this.meetsHpCondition(s, a);
        case 3:
          return this.meetsMpCondition(s, a);
        case 4:
          return this.meetsStateCondition(s);
        case 5:
          return this.meetsPartyLevelCondition(s);
        case 6:
          return this.meetsSwitchCondition(s);
        default:
          return !0;
      }
    }
    ...
    selectAction(t, e) {
      ...
      if (!(i > 0)) return null;
      {
        let s = Math.randomInt(i);
        for (const i of t) if (((s -= i.rating - e), s < 0)) return i;
      }
    }
    selectAllActions(t) {
      const e =
  ...
  class Game_CharacterBase {
    ...
    canPass(t, e, i) {
      const s = self.$gameMap.roundXWithDirection(t, i),
        a = self.$gameMap.roundYWithDirection(e, i);
      return !(
        !self.$gameMap.isValid(s, a) ||
        (!this.isThrough() &&
          !this.isDebugThrough() &&
          (!this.isMapPassable(t, e, i) || this.isCollidedWithCharacters(s, a)))
      );
    }
    canPassDiagonally(t, e, i, s) {
      const a = self.$gameMap.roundXWithDirection(t, i),
        r = self.$gameMap.roundYWithDirection(e, s);
      return (
        !(!this.canPass(t, e, s) || !this.canPass(t, r, i)) ||
        !(!this.canPass(t, e, i) || !this.canPass(a, e, s))
      );
    }
    isMapPassable(t, e, i) {
      const s = self.$gameMap.roundXWithDirection(t, i),
        ...
        r = this.reverseDir(i);
      return (
        self.$gameMap.isPassable(t, e, i) && self.$gameMap.isPassable(s, a, r)
      );
    }
    ...
    isNearTheScreen() {
      const t = Graphics$1.width,
        e = Graphics$1.height,
        i = self.$gameMap.tileWidth(),
        s = self.$gameMap.tileHeight(),
        a = this.scrolledX() * i + i / 2 - t / 2,
        r = this.scrolledY() * s + s / 2 - e / 2;
      return a >= -t && a <= t && r >= -e && r <= e;
    }
    update() {
    ...
    findDirectionTo(t, e) {
      ...
      for (
        h.parent = null,
          ...
          h.y = this.y,
          h.g = 0,
          h.f = self.$gameMap.distance(h.x, h.y, t, e),
          a.push(h),
          r.push(h.y * s + h.x);
        a.length > 0;
        ...
        let h = 0;
        for (let t = 0; t < a.length; t++) a[t].f < a[h].f && (h = t);
        const l = a[h],
          c = l.x,
          d = l.y,
          m = d * s + c,
        ...
        if (!(u >= i))
          for (let i = 0; i < 4; i++) {
            const h = 2 + 2 * i,
              m = self.$gameMap.roundXWithDirection(c, h),
              p = self.$gameMap.roundYWithDirection(d, h),
              _ = p * s + m;
            if (n.contains(_)) continue;
            if (!this.canPass(c, d, h)) continue;
            const g = u + 1,
              f = r.indexOf(_);
            if (f < 0 || g < a[f].g) {
              let i;
              f >= 0 ? (i = a[f]) : ((i = {}), a.push(i), r.push(_)),
                (i.parent = l),
                (i.x = m),
                (i.y = p),
                (i.g = g),
                (i.f = g + self.$gameMap.distance(m, p, t, e)),
                (!o || i.f - i.g < o.f - o.g) && (o = i);
            }
          }
      }
      let l = o;
      for (; l.parent && l.parent !== h; ) l = l.parent;
      const c = self.$gameMap.deltaX(l.x, h.x),
        d = self.$gameMap.deltaY(l.y, h.y);
      if (d > 0) return 2;
      if (c < 0) return 4;
      if (c > 0) return 6;
      if (d < 0) return 8;
      const m = this.deltaXFrom(t),
        u = this.deltaYFrom(e);
      return Math.abs(m) > Math.abs(u)
        ? m > 0
          ? 4
          : 6
        : 0 !== u
  ...
  class Window_ShopBuy extends Window_Selectable {
    ...
    initialize(t, e, i, s) {
      const a = this.windowWidth();
      super.initialize(t, e, a, i),
        (this._shopGoods = s),
        (this._money = 0),
        this.refresh(),
        this.select(0);
  ...
  class Window_ShopStatus extends Window_Base {
    ...
    drawPossession(t, e) {
      const i = this.contents.width - this.textPadding() - t,
        s = this.textWidth("0000");
      this.changeTextColor(this.systemColor()),
        this.drawText(TextManager.possession, t, e, i - s),
        this.resetTextColor(),
    ...
    drawEquipInfo(t, e) {
      ...
      for (let s = 0; s < i.length; s++)
        this.drawActorEquipInfo(t, e + this.lineHeight() * (2.4 * s), i[s]);
    }
    statusMembers() {
    ...
    drawActorEquipInfo(t, e, i) {
      const s = i.canEquip(this._item);
      this.changePaintOpacity(s),
        this.resetTextColor(),
        this.drawText(i.name(), t, e, 168);
      const a = this.currentEquippedItem(i, this._item.etypeId);
      s && this.drawActorParamChange(t, e, i, a),
        this.drawItemName(a, t, e + this.lineHeight()),
        this.changePaintOpacity(!0);
    }
    drawActorParamChange(t, e, i, s) {
      const a = this.contents.width - this.textPadding() - t,
        r = this.paramId(),
        n = this._item.params[r] - (s ? s.params[r] : 0);
      this.changeTextColor(this.paramchangeTextColor(n)),
        this.drawText((n > 0 ? "+" : "") + n, t, e, a, "right");
    }
    paramId() {
    ...
    currentEquippedItem(t, e) {
      const i = [],
        s = t.equips(),
        a = t.equipSlots();
      for (let t = 0; t < a.length; t++) a[t] === e && i.push(s[t]);
      const r = this.paramId();
      let n = Number.MAX_VALUE,
        h = null;
      for (let t = 0; t < i.length; t++)
        i[t] && i[t].params[r] < n && ((n = i[t].params[r]), (h = i[t]));
      return h;
    }
    update() {
  ...
  class Scene_Shop extends Scene_MenuBase {
    ...
    createDummyWindow() {
      const t = this._commandWindow.y + this._commandWindow.height,
        e = Graphics$1.boxHeight - t;
      (this._dummyWindow = new Window_Base(0, t, Graphics$1.boxWidth, e)),
    ...
    createStatusWindow() {
      const t = this._numberWindow.width,
        ...
        i = Graphics$1.boxWidth - t,
        s = this._dummyWindow.height;
      (this._statusWindow = new Window_ShopStatus(t, e, i, s)),
    ...
    createSellWindow() {
      const t = this._categoryWindow.y + this._categoryWindow.height,
        e = Graphics$1.boxHeight - t;
      (this._sellWindow = new Window_ShopSell(0, t, Graphics$1.boxWidth, e)),
  ...
  class Window_NameEdit extends Window_Base {
    ...
    initialize(t, e) {
      const i = this.windowWidth(),
        s = this.windowHeight(),
        a = (Graphics$1.boxWidth - i) / 2,
        r = (Graphics$1.boxHeight - (s + this.fittingHeight(9) + 8)) / 2;
      super.initialize(a, r, i, s),
        (this._actor = t),
  ...
  class Game_Interpreter {
    ...
    operateValue(t, e, i) {
      const s = 0 === e ? i : self.$gameVariables.value(i);
      return 0 === t ? s : -s;
    }
    changeHp(t, e, i) {
      t.isAlive() &&
    ...
    setupChoices(t) {
      const e = t[0].clone();
      let i = t[1];
      const s = t.length > 2 ? t[2] : 0,
        a = t.length > 3 ? t[3] : 2,
        r = t.length > 4 ? t[4] : 0;
      i >= e.length && (i = -2),
        self.$gameMessage.setChoices(e, s, i),
        self.$gameMessage.setChoiceBackground(r),
        self.$gameMessage.setChoicePositionType(a),
    ...
    gameDataOperand(t, e, i) {
      switch (t) {
        ...
        case 3:
          ...
          break;
        case 4:
          const s = self.$gameTroop.members()[e];
          if (s)
            switch (i) {
              case 0:
                return s.hp;
              case 1:
                return s.mp;
              default:
                if (i >= 2 && i <= 9) return s.param(i - 2);
            }
          break;
        case 5:
          const a = this.character(e);
          if (a)
            switch (i) {
              case 0:
                return a.x;
          ...
          break;
        case 6:
          const r = self.$gameParty.members()[e];
          return r ? r.actorId() : 0;
        case 7:
          switch (e) {
            case 0:
    ...
    operateVariable(t, e, i) {
      try {
        let s = self.$gameVariables.value(t);
        switch (e) {
          case 0:
            self.$gameVariables.setValue(t, (s = i));
            break;
          case 1:
            self.$gameVariables.setValue(t, s + i);
            break;
    ...
    command202() {
      ...
      const s = self.$gameMap.vehicle(this._params[0]);
      return s && s.setLocation(t, e, i), !0;
    }
    command203() {
    ...
    command261() {
      if (!self.$gameMessage.isBusy()) {
        ...
        if (t.length > 0) {
          ...
          Graphics$1.playVideo(`movies/${t}${e}`), this.setWaitMode("video");
        }
        this._index++;
      }
      return !1;
    ...
    videoFileExt() {
      return Graphics$1.canPlayVideoType("video/webm") &&
      ...
      switch (e) {
        ...
        case 337:
          if (i[1]) {
            const t = self.$dataAnimations[i[1]],
              ...
              a = t.animation1Hue,
              r = t.animation2Hue;
            ImageManager.requestAnimation(e, a),
              ImageManager.requestAnimation(s, r);
          }
          break;
        case 216:
          0 === i[0] &&
        ...
        case 336:
          const e = self.$dataEnemies[i[1]],
            ...
            a = e.battlerHue;
          self.$gameSystem.isSideView()
            ? ImageManager.requestSvEnemy(s, a)
            : ImageManager.requestEnemy(s, a);
          break;
        case 356:
          const r = i[0].split(" "),
            n = r.shift();
          Game_Interpreter.requestImagesByPluginCommand(n, r);
      }
    }
    static requestImagesByChildEvent({ parameters: t }, e) {
      const i = t,
        s = self.$dataCommonEvents[i[0]];
      s &&
        (e || (e = []),
        e.contains(i[0]) ||
          (e.push(i[0]), Game_Interpreter.requestImages(s.list, e)));
    }
        ...
        this.troop().members.forEach(function (t) {
          if (self.$dataEnemies[t.enemyId]) {
            const e = t.enemyId,
              i = t.x,
              s = t.y,
              a = new Game_Enemy(e, i, s);
            t.hidden && a.hide(), this._enemies.push(a);
          }
        }, this),
      ...
      this.members().forEach(function (e) {
        if (e.isAlive() && e.isLetterEmpty()) {
          const i = e.originalName(),
            s = this._namesCount[i] || 0;
          e.setLetter(t[s % t.length]), (this._namesCount[i] = s + 1);
        }
      }, this),
    ...
    meetsConditions({ conditions: t }) {
      ...
      if (e.turnValid) {
        const t = this._turnCount,
          ...
          s = e.turnB;
        if (0 === s && t !== i) return !1;
        if (s > 0 && (t < 1 || t < i || t % s != i % s)) return !1;
      }
      if (e.enemyValid) {
  ...
  class Game_Vehicle extends Game_Character {
    ...
    isMapPassable(t, e, i) {
      const s = self.$gameMap.roundXWithDirection(t, i),
        a = self.$gameMap.roundYWithDirection(e, i);
      return this.isBoat()
        ? self.$gameMap.isBoatPassable(s, a)
        : this.isShip()
        ? self.$gameMap.isShipPassable(s, a)
        : !!this.isAirship();
    }
    getOn() {
      (this._driving = !0),
    ...
    isLandOk(t, e, i) {
      if (this.isAirship()) {
        ...
        const s = self.$gameMap.roundXWithDirection(t, i),
          a = self.$gameMap.roundYWithDirection(e, i);
        if (!self.$gameMap.isValid(s, a)) return !1;
        if (!self.$gameMap.isPassable(s, a, this.reverseDir(i))) return !1;
        if (this.isCollidedWithCharacters(s, a)) return !1;
      }
      return !0;
    }
  }
  ...
  class Game_Map {
    ...
    distance(t, e, i, s) {
      return Math.abs(this.deltaX(t, i)) + Math.abs(this.deltaY(e, s));
    }
    canvasToMapX(t) {
    ...
    tileId(t, e, i) {
      const s = self.$dataMap.width,
        a = self.$dataMap.height;
      return self.$dataMap.data[(i * a + e) * s + t] || 0;
    }
    layeredTiles(t, e) {
      const i = [];
      for (let s = 0; s < 4; s++) i.push(this.tileId(t, e, 3 - s));
      return i;
    }
    allTiles(t, e) {
    ...
    checkLayeredTilesFlags(t, e, i) {
      const s = this.tilesetFlags();
      return this.layeredTiles(t, e).some((t) => 0 != (s[t] & i));
    }
    isLadder(t, e) {
  ...
  class Game_Follower extends Game_Character {
    ...
    chaseCharacter({ x: t, y: e }) {
      const i = this.deltaXFrom(t),
        s = this.deltaYFrom(e);
      0 !== i && 0 !== s
        ? this.moveDiagonally(i > 0 ? 4 : 6, s > 0 ? 8 : 2)
        : 0 !== i
        ? this.moveStraight(i > 0 ? 4 : 6)
        : 0 !== s && this.moveStraight(s > 0 ? 8 : 2),
        this.setMoveSpeed(self.$gamePlayer.realMoveSpeed());
    }
  }
  ...
  class Game_Player extends Game_Character {
    ...
    reserveTransfer(t, e, i, s, a) {
      (this._transferring = !0),
        (this._newMapId = t),
        (this._newX = e),
        (this._newY = i),
        (this._newDirection = s),
        (this._fadeType = a);
    }
    requestMapReload() {
    ...
    isMapPassable(t, e, i) {
      const s = this.vehicle();
      return s
        ? s.isMapPassable(t, e, i)
    ...
    centerX() {
      return (Graphics$1.width / self.$gameMap.tileWidth() - 1) / 2;
    }
    centerY() {
      return (Graphics$1.height / self.$gameMap.tileHeight() - 1) / 2;
    }
    center(t, e) {
    ...
    startMapEvent(t, e, i, s) {
      self.$gameMap.isEventRunning() ||
        self.$gameMap.eventsXy(t, e).forEach((t) => {
          t.isTriggerIn(i) && t.isNormalPriority() === s && t.start();
        });
    }
    moveByInput() {
    ...
    update(t) {
      ...
      this.updateDashing(),
        ...
        this.updateVehicle(),
        this.isMoving() || this.updateNonmoving(s),
    ...
    updateScroll(t, e) {
      const i = t,
        ...
        a = this.scrolledX(),
        r = this.scrolledY();
      r > s && r > this.centerY() && self.$gameMap.scrollDown(r - s),
        a < i && a < this.centerX() && self.$gameMap.scrollLeft(i - a),
        a > i && a > this.centerX() && self.$gameMap.scrollRight(a - i),
        r < s && r < this.centerY() && self.$gameMap.scrollUp(s - r);
    }
    updateVehicle() {
      this.isInVehicle() &&
        !this.areFollowersGathering() &&
    ...
    triggerTouchAction() {
      if (self.$gameTemp.isDestinationValid()) {
        const t = this.direction(),
          e = this.x,
          i = this.y,
          s = self.$gameMap.roundXWithDirection(e, t),
          a = self.$gameMap.roundYWithDirection(i, t),
          r = self.$gameMap.roundXWithDirection(s, t),
          n = self.$gameMap.roundYWithDirection(a, t),
          h = self.$gameTemp.destinationX(),
          o = self.$gameTemp.destinationY();
        if (h === e && o === i) return this.triggerTouchActionD1(e, i);
        if (h === s && o === a) return this.triggerTouchActionD2(s, a);
        if (h === r && o === n) return this.triggerTouchActionD3(s, a);
      }
      return !1;
    }
    ...
    checkEventTriggerThere(t) {
      if (this.canStartLocalEvents()) {
        const e = this.direction(),
          i = this.x,
          s = this.y,
          a = self.$gameMap.roundXWithDirection(i, e),
          r = self.$gameMap.roundYWithDirection(s, e);
        if (
          (this.startMapEvent(a, r, t, !0),
          !self.$gameMap.isAnyEventStarting() && self.$gameMap.isCounter(a, r))
        ) {
          const i = self.$gameMap.roundXWithDirection(a, e),
            s = self.$gameMap.roundYWithDirection(r, e);
          this.startMapEvent(i, s, t, !0);
        }
      }
    }
    checkEventTriggerTouch(t, e) {
    ...
    getOnVehicle() {
      ...
      return (
        self.$gameMap.airship().pos(e, i)
          ...
          : self.$gameMap.ship().pos(s, a)
          ? (this._vehicleType = "ship")
          : self.$gameMap.boat().pos(s, a) && (this._vehicleType = "boat"),
        this.isInVehicle() &&
  ...
  class DataManager {
    ...
    static loadDatabase() {
      ...
      for (let e = 0; e < this._databaseFiles.length; e++) {
        const i = this._databaseFiles[e].name,
          s = this._databaseFiles[e].src;
        this.loadDataFile(i, t + s);
      }
    ...
    static loadGlobalInfo() {
      ...
      if (t) {
        ...
        for (let t = 1; t <= this.maxSavefiles(); t++)
          StorageManager.exists(t) || delete this._globalInfo[t];
        return this._globalInfo;
      }
      return (this._globalInfo = []), this._globalInfo;
    }
    static saveGlobalInfo(t) {
      (this._globalInfo = null), StorageManager.save(0, JSON.stringify(t));
    }
    static isThisGameFile(t) {
      const e = this.loadGlobalInfo();
      if (e && e[t]) {
        if (StorageManager.isLocalMode()) return !0;
        {
          const i = e[t];
          return (
            i.globalId === this._globalId &&
            i.title === self.$dataSystem.gameTitle
          );
        }
      }
      return !1;
    ...
    static saveGameWithoutRescue(t) {
      const e = JsonEx.stringify(this.makeSaveContents());
      e.length, StorageManager.save(t, e), (this._lastAccessedId = t);
      const i = this.loadGlobalInfo() || [];
      return (i[t] = this.makeSavefileInfo()), this.saveGlobalInfo(i), !0;
    }
    static loadGameWithoutRescue(t) {
      if (this.isThisGameFile(t)) {
        const e = StorageManager.load(t);
        return (
          this.createGameObjects(),
  ...
  let $dataActors = null,
    ...
    $testEvent = null;
  ((t, e, i) => {
    (t.$dataActors = $dataActors),
      (t.$dataClasses = $dataClasses),
      (t.$dataSkills = $dataSkills),
      (t.$dataItems = $dataItems),
      (t.$dataWeapons = $dataWeapons),
      (t.$dataArmors = $dataArmors),
      (t.$dataEnemies = $dataEnemies),
      (t.$dataTroops = $dataTroops),
      (t.$dataStates = $dataStates),
      (t.$dataAnimations = $dataAnimations),
      (t.$dataTilesets = $dataTilesets),
      (t.$dataCommonEvents = $dataCommonEvents),
      (t.$dataSystem = $dataSystem),
      (t.$dataMapInfos = $dataMapInfos),
      (t.$dataMap = $dataMap),
      (t.$gameTemp = $gameTemp),
      (t.$gameSystem = $gameSystem),
      (t.$gameScreen = $gameScreen),
      (t.$gameTimer = $gameTimer),
      (t.$gameMessage = $gameMessage),
      (t.$gameSwitches = $gameSwitches),
      (t.$gameVariables = $gameVariables),
      (t.$gameSelfSwitches = $gameSelfSwitches),
      (t.$gameActors = $gameActors),
      (t.$gameParty = $gameParty),
      (t.$gameTroop = $gameTroop),
      (t.$gameMap = $gameMap),
      (t.$gamePlayer = $gamePlayer),
      (t.$testEvent = $testEvent),
      (t.Game_Temp = Game_Temp),
      (t.Game_System = Game_System),
      (t.Game_Timer = Game_Timer),
      (t.Game_Message = Game_Message),
      (t.Game_Switches = Game_Switches),
      (t.Game_Variables = Game_Variables),
      (t.Game_SelfSwitches = Game_SelfSwitches),
      (t.Game_Screen = Game_Screen),
      (t.Game_Picture = Game_Picture),
      (t.Game_Item = Game_Item),
      (t.Game_Action = Game_Action),
      (t.Game_ActionResult = Game_ActionResult),
      (t.Game_BattlerBase = Game_BattlerBase),
      (t.Game_Battler = Game_Battler),
      (t.Game_Actor = Game_Actor),
      (t.Game_Enemy = Game_Enemy),
      (t.Game_Actors = Game_Actors),
      (t.Game_Unit = Game_Unit),
      (t.Game_Party = Game_Party),
      (t.Game_Troop = Game_Troop),
      (t.Game_Map = Game_Map),
      (t.Game_CommonEvent = Game_CommonEvent),
      (t.Game_CharacterBase = Game_CharacterBase),
      (t.Game_Character = Game_Character),
      (t.Game_Player = Game_Player),
      (t.Game_Follower = Game_Follower),
      (t.Game_Followers = Game_Followers),
      (t.Game_Vehicle = Game_Vehicle),
      (t.Game_Event = Game_Event),
      (t.Game_Interpreter = Game_Interpreter),
      (t.Scene_Base = Scene_Base),
      (t.Scene_Boot = Scene_Boot),
      (t.Scene_Title = Scene_Title),
      (t.Scene_Map = Scene_Map),
      (t.Scene_MenuBase = Scene_MenuBase),
      (t.Scene_Menu = Scene_Menu),
      (t.Scene_ItemBase = Scene_ItemBase),
      (t.Scene_Item = Scene_Item),
      (t.Scene_Skill = Scene_Skill),
      (t.Scene_Equip = Scene_Equip),
      (t.Scene_Status = Scene_Status),
      (t.Scene_Options = Scene_Options),
      (t.Scene_File = Scene_File),
      (t.Scene_Save = Scene_Save),
      (t.Scene_Load = Scene_Load),
      (t.Scene_GameEnd = Scene_GameEnd),
      (t.Scene_Shop = Scene_Shop),
      (t.Scene_Name = Scene_Name),
      (t.Scene_Debug = Scene_Debug),
      (t.Scene_Battle = Scene_Battle),
      (t.Scene_Gameover = Scene_Gameover),
      (t.Sprite_Base = Sprite_Base),
      (t.Sprite_Button = Sprite_Button),
      (t.Sprite_Character = Sprite_Character),
      (t.Sprite_Battler = Sprite_Battler),
      (t.Sprite_Actor = Sprite_Actor),
      (t.Sprite_Enemy = Sprite_Enemy),
      (t.Sprite_Animation = Sprite_Animation),
      (t.Sprite_Damage = Sprite_Damage),
      (t.Sprite_StateIcon = Sprite_StateIcon),
      (t.Sprite_StateOverlay = Sprite_StateOverlay),
      (t.Sprite_Weapon = Sprite_Weapon),
      (t.Sprite_Balloon = Sprite_Balloon),
      (t.Sprite_Picture = Sprite_Picture),
      (t.Sprite_Timer = Sprite_Timer),
      (t.Sprite_Destination = Sprite_Destination),
      (t.Spriteset_Base = Spriteset_Base),
      (t.Spriteset_Map = Spriteset_Map),
      (t.Spriteset_Battle = Spriteset_Battle),
      (t.Window_Base = Window_Base),
      (t.Window_Selectable = Window_Selectable),
      (t.Window_Command = Window_Command),
      (t.Window_HorzCommand = Window_HorzCommand),
      (t.Window_Help = Window_Help),
      (t.Window_Gold = Window_Gold),
      (t.Window_MenuCommand = Window_MenuCommand),
      (t.Window_MenuStatus = Window_MenuStatus),
      (t.Window_MenuActor = Window_MenuActor),
      (t.Window_ItemCategory = Window_ItemCategory),
      (t.Window_ItemList = Window_ItemList),
      (t.Window_SkillType = Window_SkillType),
      (t.Window_SkillStatus = Window_SkillStatus),
      (t.Window_SkillList = Window_SkillList),
      (t.Window_EquipStatus = Window_EquipStatus),
      (t.Window_EquipCommand = Window_EquipCommand),
      (t.Window_EquipSlot = Window_EquipSlot),
      (t.Window_EquipItem = Window_EquipItem),
      (t.Window_Status = Window_Status),
      (t.Window_Options = Window_Options),
      (t.Window_SavefileList = Window_SavefileList),
      (t.Window_ShopCommand = Window_ShopCommand),
      (t.Window_ShopBuy = Window_ShopBuy),
      (t.Window_ShopSell = Window_ShopSell),
      (t.Window_ShopNumber = Window_ShopNumber),
      (t.Window_ShopStatus = Window_ShopStatus),
      (t.Window_NameEdit = Window_NameEdit),
      (t.Window_NameInput = Window_NameInput),
      (t.Window_ChoiceList = Window_ChoiceList),
      (t.Window_NumberInput = Window_NumberInput),
      (t.Window_EventItem = Window_EventItem),
      (t.Window_Message = Window_Message),
      (t.Window_ScrollText = Window_ScrollText),
      (t.Window_MapName = Window_MapName),
      (t.Window_BattleLog = Window_BattleLog),
      (t.Window_PartyCommand = Window_PartyCommand),
      (t.Window_ActorCommand = Window_ActorCommand),
      (t.Window_BattleStatus = Window_BattleStatus),
      (t.Window_BattleActor = Window_BattleActor),
      (t.Window_BattleEnemy = Window_BattleEnemy),
      (t.Window_BattleSkill = Window_BattleSkill),
      (t.Window_BattleItem = Window_BattleItem),
      (t.Window_TitleCommand = Window_TitleCommand),
      (t.Window_GameEnd = Window_GameEnd),
      (t.Window_DebugRange = Window_DebugRange),
      (t.Window_DebugEdit = Window_DebugEdit),
      (t.ProgressWatcher = ProgressWatcher$1),
      (t.Utils = Utils),
      (t.CacheEntry = CacheEntry),
      (t.CacheMap = CacheMap),
      (t.ImageCache = ImageCache),
      (t.RequestQueue = RequestQueue),
      (t.Point = Point),
      (t.Rectangle = Rectangle),
      (t.Bitmap = Bitmap),
      (t.BitmapPIXI = BitmapPIXI),
      (t.Graphics = Graphics$1),
      (t.Input = Input),
      (t.TouchInput = TouchInput),
      (t.Sprite = Sprite),
      (t.Tilemap = Tilemap),
      (t.ShaderTilemap = ShaderTilemap),
      (t.TilingSprite = TilingSprite),
      (t.ScreenSprite = ScreenSprite),
      (t.WindowSkinCache = WindowSkinCache),
      (t.Window = Window),
      (t.WindowLayer = WindowLayer),
      (t.Weather = Weather),
      (t.ToneFilter = ToneFilter),
      (t.ToneSprite = ToneSprite),
      (t.Stage = Stage),
      (t.WebAudio = WebAudio),
      (t.JsonEx = JsonEx),
      (t.Decrypter = Decrypter),
      (t.ResourceHandler = ResourceHandler),
      (t.DataManager = DataManager),
      (t.ConfigManager = ConfigManager),
      (t.StorageManager = StorageManager),
      (t.ImageManager = ImageManager),
      (t.AudioManager = AudioManager),
      (t.SoundManager = SoundManager),
      (t.TextManager = TextManager),
      (t.SceneManager = SceneManager),
      (t.BattleManager = BattleManager),
      (t.PluginManager = PluginManager),
      PluginManager.setup($plugins);
    const s = () => {
      e.body.classList.remove("is-loading"),
        (i.settings.SCALE_MODE = i.SCALE_MODES.NEAREST),
        (i.settings.ROUND_PIXELS = !0),
        (i.settings.GC_MAX_IDLE = 600),
        (i.settings.MIPMAP_TEXTURES = i.MIPMAP_MODES.OFF),
        (i.settings.RESOLUTION = window.devicePixelRatio),
        Utils.isMobileSafari() &&
          (i.settings.PRECISION_FRAGMENT = i.PRECISION.HIGH),
        SceneManager.run(Scene_Boot);
    };
    "complete" === e.readyState ? s() : window.addEventListener("load", s);
  })(self, document, PIXI);
})();
